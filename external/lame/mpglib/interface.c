...
check_vbr_header( PMPSTR mp, int bytes )
{
    ...
    if ( mp->vbr_header )
    {
        ...
        /* lame_report_fnc(mp->report_msg,"hip: Xing VBR header dectected.  MP3 file has %i
         * frames\n", pTagData.frames); */
...
audiodata_precedesframes( PMPSTR mp )
{
    if ( mp->fr.lay == 3 )
        ...
        return 0; /* For Layer 1 & 2 the audio data starts at the frame that describes it, so no
                     audio data precedes. */
}

static int
decodeMP3_clipchoice( PMPSTR mp,
                      unsigned char* in,
                      int isize,
                      char* out,
                      int* done,
                      ...
                      int ( *synth_1to1_ptr )( PMPSTR, real*, int, unsigned char*, int* ) )
{
    ...
    if ( !mp->header_parsed )
    {
        if ( mp->fsizeold == -1 || mp->sync_bitstream )
        {
            ...
            if ( mp->vbr_header )
            {
                ...
                {
                    /* lame_report_fnc(mp->report_err,"hip: not enough data to parse entire Xing
                     * header\n"); */
                    return MP3_NEED_MORE;
                }
...
#ifdef HIP_DEBUG
            lame_report_fnc( mp->report_dbg,
                             "hip: %d bits needed to parse layer III frame, number of bytes to "
                             "read before decoding dsize = %d\n",
                             bits,
                             mp->dsize );
#endif
    ...
    if ( !mp->data_parsed )
    {
        ...
        *done = 0;
        ...
        case 3:
            decode_layer3_frame(
                mp, (unsigned char*)out, done, synth_1to1_mono_ptr, synth_1to1_ptr );
            break;
        default:
...
decodeMP3( PMPSTR mp, unsigned char* in, int isize, char* out, int osize, int* done )
{
    ...
    {
        lame_report_fnc(
            mp->report_err, "hip: Insufficient memory for decoding buffer %d\n", osize );
        return MP3_ERR;
    }
...
decodeMP3_unclipped( PMPSTR mp, unsigned char* in, int isize, char* out, int osize, int* done )
{
    ...
    return decodeMP3_clipchoice(
        mp, in, isize, out, done, synth_1to1_mono_unclipped, synth_1to1_unclipped );
}
