...
static unsigned char grp_3tab[ 32 * 3 ] = {
    0,
}; /* used: 27 */
static unsigned char grp_5tab[ 128 * 3 ] = {
    0,
}; /* used: 125 */
static unsigned char grp_9tab[ 1024 * 3 ] = {
    0,
}; /* used: 729 */
...
hip_init_tables_layer2( void )
{
    static const double mulmul[ 27 ]
        = {0.0,          -2.0 / 3.0,   2.0 / 3.0,     2.0 / 7.0,     2.0 / 15.0,    2.0 / 31.0,
           2.0 / 63.0,   2.0 / 127.0,  2.0 / 255.0,   2.0 / 511.0,   2.0 / 1023.0,  2.0 / 2047.0,
           2.0 / 4095.0, 2.0 / 8191.0, 2.0 / 16383.0, 2.0 / 32767.0, 2.0 / 65535.0, -4.0 / 5.0,
           -2.0 / 5.0,   2.0 / 5.0,    4.0 / 5.0,     -8.0 / 9.0,    -4.0 / 9.0,    -2.0 / 9.0,
...
static void
II_step_two(
    PMPSTR mp, sideinfo_layer_II* si, struct frame* fr, int gr, real fraction[ 2 ][ 4 ][ SBLIMIT ] )
{
    struct al_table2 const* alloc1 = fr->alloc;
    int sblimit = fr->II_sblimit;
    int jsbound = ( fr->mode == MPG_MD_JOINT_STEREO ) ? ( fr->mode_ext << 2 ) + 4 : fr->II_sblimit;
    ...
    for ( i = 0; i < jsbound; ++i )
    {
        ...
        for ( ch = 0; ch < nch; ++ch )
        {
            ...
            {
                fraction[ ch ][ 0 ][ i ] = fraction[ ch ][ 1 ][ i ] = fraction[ ch ][ 2 ][ i ]
                    = 0.0;
            }
        }
        alloc1 += ( (size_t)1 << step );
