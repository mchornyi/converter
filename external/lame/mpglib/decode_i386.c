...
#define WRITE_SAMPLE_CLIPPED( TYPE, samples, sum, clip )                    \
    ...
    {                                                                       \
        *( samples ) = ( TYPE )( ( sum ) > 0 ? ( sum ) + 0.5 : (sum)-0.5 ); \
    }

#define WRITE_SAMPLE_UNCLIPPED( TYPE, samples, sum, clip ) *samples = (TYPE)sum;

/* *INDENT-OFF* */

/* versions: clipped (when TYPE == short) and unclipped (when TYPE == real) of synth_1to1_mono*
 * functions */
