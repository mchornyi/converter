...
const unsigned int AEncodeProperties::the_Bitrates[ 18 ]
    = {320, 256, 224, 192, 160, 144, 128, 112, 96, 80, 64, 56, 48, 40, 32, 24, 16, 8};
const unsigned int AEncodeProperties::the_MPEG1_Bitrates[ 14 ]
    = {320, 256, 224, 192, 160, 128, 112, 96, 80, 64, 56, 48, 40, 32};
const unsigned int AEncodeProperties::the_MPEG2_Bitrates[ 14 ]
...
// const char         AEncodeProperties::the_Presets[][13] = {"None", "CD", "Studio", "Hi-Fi",
// "Phone", "Voice", "Radio", "Tape", "FM", "AM", "SW"}; const LAME_QUALTIY_PRESET
// AEncodeProperties::the_Presets[] = {LQP_NOPRESET, LQP_R3MIX_QUALITY, LQP_NORMAL_QUALITY,
// LQP_LOW_QUALITY, LQP_HIGH_QUALITY, LQP_VERYHIGH_QUALITY, LQP_VOICE_QUALITY, LQP_PHONE, LQP_SW,
// LQP_AM, LQP_FM, LQP_VOICE, LQP_RADIO, LQP_TAPE, LQP_HIFI, LQP_CD, LQP_STUDIO}; const unsigned int
// AEncodeProperties::the_SamplingFreqs[9] = { 48000, 44100, 32000, 24000, 22050, 16000, 12000,
// 11025, 8000 };

ToolTipItem AEncodeProperties::Tooltips[ 13 ] = {
    {IDC_CHECK_ENC_ABR,
     "Allow encoding with an average bitrate\r\ninstead of a constant one.\r\n\r\nIt can improve "
     "the quality for the same bitrate."},
    {IDC_CHECK_COPYRIGHT, "Mark the encoded data as copyrighted."},
    {IDC_CHECK_CHECKSUM,
     "Put a checksum in the encoded data.\r\n\r\nThis can make the file less sensitive to data "
     "loss."},
    {IDC_CHECK_ORIGINAL, "Mark the encoded data as an original file."},
    {IDC_CHECK_PRIVATE, "Mark the encoded data as private."},
    {IDC_COMBO_ENC_STEREO,
     "Select the type of stereo mode used for encoding:\r\n\r\n- Stereo : the usual one\r\n- "
     "Joint-Stereo : mix both channel to achieve better compression\r\n- Dual Channel : treat both "
     "channel as separate"},
    {IDC_STATIC_DECODING, "Decoding not supported for the moment by the codec."},
    {IDC_CHECK_ENC_SMART,
     "Disable bitrate when there is too much compression.\r\n(default 1:15 ratio)"},
    {IDC_STATIC_CONFIG_VERSION,
     "Version of this codec.\r\n\r\nvX.X.X is the version of the codec interface.\r\nX.XX is the "
     "version of the encoding engine."},
    ...
    {IDC_SLIDER_AVERAGE_STEP,
     "Select the step of Average Bitrate between the min and max.\r\n\r\nA step of 5 between 152 "
     "and 165 means you have :\r\n165, 160 and 155"},
    {IDC_SLIDER_AVERAGE_SAMPLE,
     "Check the resulting values of the (min,max,step) combination.\r\n\r\nUse the keyboard to "
     "navigate (right -> left)."},
};
...
(LPARAM)the_prop->GetOutputDirectory());
...
//		if
(CreateFile(FolderName,STANDARD_RIGHTS_WRITE,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL) ==
INVALID_HANDLE_VALUE) if ((GetFileAttributes(FolderName) & FILE_ATTRIBUTE_DIRECTORY) != 0)
...
ConfigProc( HWND hwndDlg,   // handle to dialog box
            UINT uMsg,      // message
...
)
{
    ...
    case WM_NOTIFY:
        if ( TTN_GETDISPINFO == ( (LPNMHDR)lParam )->code )
        {
            ...
            UINT id = ( lphdr->uFlags & TTF_IDISHWND )
                          ? GetWindowLong( (HWND)lphdr->hdr.idFrom, GWL_ID )
                          : lphdr->hdr.idFrom;
            ...
            *lphdr->lpszText = 0;
            ...
            for ( int i = 0;
                  i < sizeof AEncodeProperties::Tooltips / sizeof AEncodeProperties::Tooltips[ 0 ];
                  ++i )
            {
...
const char*
AEncodeProperties::GetChannelModeString( int a_channelID ) const
{
...
const int
AEncodeProperties::GetBitrateString( char* string, int string_size, int a_bitrateID ) const
{
...
const unsigned int
AEncodeProperties::GetChannelModeValue( ) const
{
...
const unsigned int
AEncodeProperties::GetBitrateValue( ) const
{
...
inline const int
AEncodeProperties::GetBitrateValueMPEG2( DWORD& bitrate ) const
{
    int i;
...
inline const int
AEncodeProperties::GetBitrateValueMPEG1( DWORD& bitrate ) const
{
    int i;
...
*/
bool
AEncodeProperties::Config( const HINSTANCE Hinstance, const HWND HwndParent )
{
    // WM_INITDIALOG ?
    ...
    int ret = ::DialogBoxParam(
        Hinstance, MAKEINTRESOURCE( IDD_CONFIG ), HwndParent, ::ConfigProc, (LPARAM)this );
    /*	if (ret == -1)
        {
            LPVOID lpMsgBuf;
            FormatMessage(
...
bool
AEncodeProperties::InitConfigDlg( HWND HwndDlg )
{
    ...
    for ( i = 0; i < GetChannelLentgh( ); i++ )
        SendMessage( GetDlgItem( HwndDlg, IDC_COMBO_ENC_STEREO ),
                     CB_ADDSTRING,
                     NULL,
                     (LPARAM)GetChannelModeString( i ) );
    ...
    */
    ...
    /*	SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_BITRATE), CB_RESETCONTENT , NULL, NULL);
        ...
        for (i=0;i<GetPresetLentgh();i++)
            SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_PRESET), CB_ADDSTRING, NULL, (LPARAM)
       GetPresetModeString(i));
    */

    // Add ABR Sliders
    SendMessage(
        GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_MIN ), TBM_SETRANGE, TRUE, MAKELONG( 8, 320 ) );
    SendMessage(
        GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_MAX ), TBM_SETRANGE, TRUE, MAKELONG( 8, 320 ) );
    SendMessage(
        GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_STEP ), TBM_SETRANGE, TRUE, MAKELONG( 1, 16 ) );
...
bool
AEncodeProperties::UpdateDlgFromValue( HWND HwndDlg )
{
    // get all the required strings
    //	TCHAR Version[5];
    //	LoadString(hDllInstance, IDS_STRING_VERSION, Version, 5);
    ...
    ::CheckDlgButton(
        HwndDlg, IDC_CHECK_ORIGINAL, GetOriginalMode( ) ? BST_CHECKED : BST_UNCHECKED );
    ::CheckDlgButton( HwndDlg, IDC_CHECK_PRIVATE, GetPrivateMode( ) ? BST_CHECKED : BST_UNCHECKED );
    ::CheckDlgButton(
        HwndDlg, IDC_CHECK_COPYRIGHT, GetCopyrightMode( ) ? BST_CHECKED : BST_UNCHECKED );
    ::CheckDlgButton(
        HwndDlg, IDC_CHECK_ENC_SMART, GetSmartOutputMode( ) ? BST_CHECKED : BST_UNCHECKED );
    ::CheckDlgButton(
        HwndDlg, IDC_CHECK_ENC_ABR, GetAbrOutputMode( ) ? BST_CHECKED : BST_UNCHECKED );
    //	::CheckDlgButton( HwndDlg, IDC_CHECK_RESERVOIR,    !GetNoBiResMode()
    //?BST_CHECKED:BST_UNCHECKED );
    //	::CheckDlgButton( HwndDlg, IDC_CHECK_XINGVBR, GetXingFrameMode()?BST_CHECKED:BST_UNCHECKED
    //);
    //	::CheckDlgButton( HwndDlg, IDC_CHECK_RESAMPLE,     GetResampleMode()
    //?BST_CHECKED:BST_UNCHECKED );
    ...
    // Add VBR Quality
    SendMessage(
        GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_MIN ), TBM_SETPOS, TRUE, AverageBitrate_Min );
    SendMessage(
        GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_MAX ), TBM_SETPOS, TRUE, AverageBitrate_Max );
    SendMessage(
        GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_STEP ), TBM_SETPOS, TRUE, AverageBitrate_Step );
    SendMessage(
        GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_SAMPLE ), TBM_SETPOS, TRUE, AverageBitrate_Max );
    ...
    // Add all possible re-sampling freq
    /*	SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_SAMPLEFREQ), CB_SETCURSEL, nSamplingFreqIndex,
    NULL);
...
bool
AEncodeProperties::UpdateValueFromDlg( HWND HwndDlg )
{
    nChannelIndex
        = SendMessage( GetDlgItem( HwndDlg, IDC_COMBO_ENC_STEREO ), CB_GETCURSEL, NULL, NULL );
    //	nMinBitrateIndex   = SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_BITRATE),    CB_GETCURSEL,
    //NULL, NULL); 	nMaxBitrateIndex   = SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_MAXBITRATE),
    //CB_GETCURSEL, NULL, NULL); 	nPresetIndex       = SendMessage(GetDlgItem( HwndDlg,
    //IDC_COMBO_PRESET),     CB_GETCURSEL, NULL, NULL); 	VbrQuality         = SendMessage(GetDlgItem(
    //HwndDlg, IDC_SLIDER_QUALITY), TBM_GETPOS , NULL, NULL); 	nSamplingFreqIndex =
    //SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_SAMPLEFREQ), CB_GETCURSEL, NULL, NULL);
    ...
    AverageBitrate_Min
        = SendMessage( GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_MIN ), TBM_GETPOS, NULL, NULL );
    AverageBitrate_Max
        = SendMessage( GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_MAX ), TBM_GETPOS, NULL, NULL );
    AverageBitrate_Step
        = SendMessage( GetDlgItem( HwndDlg, IDC_SLIDER_AVERAGE_STEP ), TBM_GETPOS, NULL, NULL );

    EnableAbrOptions( HwndDlg, bAbrOutput );

    my_debug.OutPut( "nChannelIndex %d, bCRC %d, bCopyright %d, bOriginal %d, bPrivate %d",
                     nChannelIndex,
                     bCRC,
                     bCopyright,
                     bOriginal,
...
/*
VBRMETHOD AEncodeProperties::GetVBRValue(DWORD & MaxBitrate, int & Quality, DWORD & AbrBitrate, BOOL
& VBRHeader, const DWORD MPEG_Version) const
{
    assert((MPEG_Version == MPEG1) || (MPEG_Version == MPEG2));
    ...
    if (mBRmode == BR_VBR)
    {
        ...
        if (MPEG_Version == MPEG1)
            MaxBitrate = MaxBitrate>the_MPEG1_Bitrates[sizeof(the_MPEG1_Bitrates)/sizeof(unsigned
int)-1]?MaxBitrate:the_MPEG1_Bitrates[sizeof(the_MPEG1_Bitrates)/sizeof(unsigned int)-1]; else
...
*/
void
AEncodeProperties::ParamsRestore( )
{
...
AEncodeProperties::ParamsSave( )
{
    /*
        ...
        if (RegCreateKeyEx ( HKEY_LOCAL_MACHINE, "SOFTWARE\\MUKOLI\\out_lame", 0, "",
       REG_OPTION_NON_VOLATILE, KEY_WRITE , NULL, &OssKey, NULL ) == ERROR_SUCCESS) {

            if (RegSetValueEx(OssKey, "DLL Location", 0, REG_EXPAND_SZ, (CONST BYTE *)DllLocation,
       strlen(DllLocation)+1 ) != ERROR_SUCCESS) return;
...
void AEncodeProperties::DisplayVbrOptions(const HWND hDialog, const BRMode the_mode)
{
    ...
    {
        case BR_CBR:
            ::CheckRadioButton(hDialog, IDC_RADIO_BITRATE_CBR, IDC_RADIO_BITRATE_ABR,
IDC_RADIO_BITRATE_CBR); break; case BR_VBR:
            ::CheckRadioButton(hDialog, IDC_RADIO_BITRATE_CBR, IDC_RADIO_BITRATE_ABR,
IDC_RADIO_BITRATE_VBR); bVBR = true; break; case BR_ABR:
            ::CheckRadioButton(hDialog, IDC_RADIO_BITRATE_CBR, IDC_RADIO_BITRATE_ABR,
IDC_RADIO_BITRATE_ABR); bABR = true; break;
    ...
    , my_hModule( hModule )
{
    ...
    HANDLE hFile = ::CreateFile( my_store_location.c_str( ),
                                 0,
...
void
AEncodeProperties::SaveValuesToStringKey( const std::string& config_name )
{
...
void
AEncodeProperties::GetValuesFromKey( const std::string& config_name, const TiXmlNode& parentNode )
{
    TiXmlElement* tmpElt;
...
/*	int nIdx = SendMessage(::GetDlgItem( hParentWnd ,IDC_COMBO_SETTINGS ), CB_GETCURSEL, NULL,
NULL);
    ::SendMessage(::GetDlgItem( hParentWnd ,IDC_COMBO_SETTINGS ), CB_GETLBTEXT , nIdx, (LPARAM)
string);
* /
}*/

bool
AEncodeProperties::operator!=( const AEncodeProperties& the_instance ) const
{
    /*
    ...
    */
    return ( ( bCopyright != the_instance.bCopyright ) || ( bCRC != the_instance.bCRC )
             || ( bOriginal != the_instance.bOriginal ) || ( bPrivate != the_instance.bPrivate )
             ...
             || ( bNoBitRes != the_instance.bNoBitRes ) || ( mBRmode != the_instance.mBRmode )
...
void
AEncodeProperties::SelectSavedParams( const std::string the_string )
{
    // get the values from the saved file if possible
    if ( my_stored_data.LoadFile( my_store_location ) )
    {
...
inline void
AEncodeProperties::SetAttributeBool( TiXmlElement* the_elt,
                                     const std::string& the_string,
                                     const bool the_value ) const
{
    if ( the_value == false )
...
void
AEncodeProperties::SaveValuesToElement( TiXmlElement* the_element ) const
{
    // get all the parameters saved in this Element
    TiXmlElement* tmpElt;
...
bool
AEncodeProperties::HandleDialogCommand( const HWND parentWnd,
                                        const WPARAM wParam,
                                        const LPARAM lParam )
{
    UINT command;
    ...
    case IDOK:
    {
        ...
        /*
                if (tmpDlgProps != tmpSavedProps)
                {
                    ...
                    else
                    {
                        ...
                        save = AOut::MyMessageBox( tmpDsp, MB_YESNOCANCEL|MB_ICONQUESTION,
        parentWnd);
                    }
        ...
        */
        //#endif // OLD
        my_debug.OutPut(
            "before : nChannelIndex %d, bCRC %d, bCopyright %d, bOriginal %d, bPrivate %d",
            nChannelIndex,
            bCRC,
            bCopyright,
            bOriginal,
        ...
        /*	case IDC_FIND_DLL:
            {
                ...
                file.Flags = OFN_FILEMUSTEXIST | OFN_NODEREFERENCELINKS | OFN_ENABLEHOOK |
        OFN_EXPLORER ;
        ///				file.lpstrFile = AOut::the_AOut->DllLocation;
                file.lpstrFile = DllLocation;
                file.lpstrFilter = "Lame DLL (lame_enc.dll)\0LAME_ENC.DLL\0DLL (*.dll)\0*.DLL\0All
        (*.*)\0*.*\0"; file.nFilterIndex = 1; file.nMaxFile  = sizeof(DllLocation); file.lpfnHook  =
        DLLFindCallback; // use to validate the DLL chosen
    ...
    case IDC_CHECK_ENC_ABR:
        EnableAbrOptions( parentWnd,
                          ::IsDlgButtonChecked( parentWnd, IDC_CHECK_ENC_ABR ) == BST_CHECKED );
        break;
        /*	case IDC_RADIO_BITRATE_CBR:
            ...
            {
                bool tmp_bResampleUsed = (::IsDlgButtonChecked( parentWnd, IDC_CHECK_RESAMPLE) ==
           BST_CHECKED); if (tmp_bResampleUsed)
                {
...
bool
AEncodeProperties::RenameCurrentTo( const std::string& new_config_name )
{
    bool bResult = false;
...
bool
AEncodeProperties::DeleteConfig( const std::string& config_name )
{
    bool bResult = false;
...
void
AEncodeProperties::UpdateConfigs( const HWND HwndDlg )
{
    // Add User configs
    //	SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_SETTINGS), CB_RESETCONTENT , NULL, NULL);
    ...
    if ( my_stored_data.LoadFile( my_store_location ) )
    {
        ...
        while ( iterateElmt != NULL )
        {
            ...
            */
            if ( tmpname != NULL )
            {
                //				SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_SETTINGS), CB_ADDSTRING, NULL,
                //(LPARAM) tmpname->c_str());
                if ( tmpname->compare( CurrentConfig ) == 0 )
                {
                    //					SendMessage(GetDlgItem( HwndDlg, IDC_COMBO_SETTINGS), CB_SETCURSEL,
                    //Idx, NULL);
...
*/
void
AEncodeProperties::UpdateDlgFromSlides( HWND hwndDlg ) const
{
    ...
    SendMessage(
        GetDlgItem( hwndDlg, IDC_SLIDER_AVERAGE_SAMPLE ), TBM_SETRANGEMIN, TRUE, value_min );

    wsprintf( tmp, "%3d", value_max );
    ::SetWindowText( GetDlgItem( hwndDlg, IDC_STATIC_AVERAGE_MAX_VALUE ), tmp );

    SendMessage(
        GetDlgItem( hwndDlg, IDC_SLIDER_AVERAGE_SAMPLE ), TBM_SETRANGEMAX, TRUE, value_max );
...
void
AEncodeProperties::EnableAbrOptions( HWND hDialog, bool enable )
{
