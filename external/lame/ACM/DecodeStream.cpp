...
const bool
DecodeStream::Erase( const DecodeStream* a_ACMStream )
{
    delete a_ACMStream;
    return true;
}
...
bool
DecodeStream::init( const int nSamplesPerSec,
                    const int nChannels,
                    const int nAvgBytesPerSec,
                    const int nSourceBitrate )
{
    bool bResult = false;
...
bool
DecodeStream::close( LPBYTE pOutputBuffer, DWORD* pOutputSize )
{
    bool bResult = false;
    /*
...
DWORD
DecodeStream::GetOutputSizeForInput( const DWORD the_SrcLength ) const
{
    DWORD Result;

    double OutputInputRatio
        = double( my_SamplesPerSec * 2 * my_Channels ) / double( my_SourceBitrate );
    ...
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_CODE, "Result = %d (OutputInputRatio = %f)", Result, OutputInputRatio );

    return Result;
}

bool
DecodeStream::ConvertBuffer( LPACMDRVSTREAMHEADER a_StreamHeader )
{
    bool result = false;
    ...
    int ret = decodeMP3( &my_DecodeData,
                         a_StreamHeader->pbSrc,
                         a_StreamHeader->cbSrcLength,
                         (char*)a_StreamHeader->pbDst,
    ...
    /*
        DWORD InSize = a_StreamHeader->cbSrcLength / 2, OutSize = a_StreamHeader->cbDstLength; // 2
    for 8<->16 bits
    ...
    int dwSamples;
        ...
        {
            nOutputSamples =
    lame_encode_buffer(gfp,(PSHORT)a_StreamHeader->pbSrc,(PSHORT)a_StreamHeader->pbSrc,dwSamples,a_StreamHeader->pbDst,a_StreamHeader->cbDstLength);
        }
        else
        {
            nOutputSamples =
    lame_encode_buffer_interleaved(gfp,(PSHORT)a_StreamHeader->pbSrc,dwSamples,a_StreamHeader->pbDst,a_StreamHeader->cbDstLength);
        }
    ...
    */
    my_debug->OutPut(
        ...
        "UsedSize = %d / EncodedSize = %d, result = %d, ret = %s",
    ...
    {
        my_debug->OutPut(
            DEBUG_LEVEL_FUNC_DEBUG, "DecodeStream::ConvertBuffer result = %d", result );
    }
