...
static const unsigned int mpeg1_bitrate[]
    = {320, 256, 224, 192, 160, 128, 112, 96, 80, 64, 56, 48, 40, 32};
static const unsigned int mpeg2_bitrate[]
...
static const int FORMAT_TAG_MAX_NB
    = 2;  // PCM and PERSONAL (mandatory to have at least PCM and your format)
static const int FILTER_TAG_MAX_NB = 0;  // this is a codec, not a filter

// number of supported PCM formats
static const int FORMAT_MAX_NB_PCM = 2 *  // number of PCM channel mode (stereo/mono)
...
//////////////////////////////////////////////////////////////////////
bool
bitrate_item::operator<( const bitrate_item& other_bitrate ) const
{
...
{
    my_debug.OutPut(DEBUG_LEVEL_FUNC_START, "ACM : Configure (Parent Window =
0x%08X)",hParentWindow);

    DialogBoxParam( my_hModule, MAKEINTRESOURCE(IDD_CONFIG), hParentWindow, ::ConfigProc ,
(LPARAM)this);
...
AboutProc( HWND hwndDlg,   // handle to dialog box
           UINT uMsg,      // message
           WPARAM wParam,  // first message parameter
           LPARAM lParam   // second message parameter
)
{
    ...
    case WM_INITDIALOG:
        ...
        /*
                    ...
                    ::SendMessage(::GetDlgItem(hwndDlg,IDC_STATIC_ABOUT_URL), WM_SETFONT, (WPARAM)
        hfnt, TRUE);
        * /
                    hBrushStatic = ::CreateSolidBrush(::GetSysColor (COLOR_BTNFACE));
        */
        hcOverCursor
            = ::LoadCursor( NULL, (LPCTSTR)IDC_HAND );
    ...
    case WM_LBUTTONUP:
    {
        ...
        {
            LPSTR tmpStr;
            HRESULT hresult = ::TranslateURL(
                Url, TRANSLATEURL_FL_GUESS_PROTOCOL | TRANSLATEURL_FL_GUESS_PROTOCOL, &tmpStr );
            if ( hresult == S_OK )
...
inline DWORD
ACM::About( HWND hParentWindow )
{
    my_debug.OutPut(
        DEBUG_LEVEL_FUNC_START, "ACM : About (Parent Window = 0x%08X)", hParentWindow );

    DialogBoxParam(
        my_hModule, MAKEINTRESOURCE( IDD_ABOUT ), hParentWindow, ::AboutProc, (LPARAM)this );
    ...
    , my_EncodingProperties( hModule )
{
    ...
    HKEY OssKey;
    if ( RegOpenKeyEx( HKEY_LOCAL_MACHINE, "SOFTWARE\\MUKOLI", 0, KEY_READ, &OssKey )
         == ERROR_SUCCESS )
    {
        DWORD DataType;
        DWORD DebugFileNameSize = 512;
        if ( RegQueryValueEx(
                 OssKey, "DebugFile", NULL, &DataType, DebugFileName, &DebugFileNameSize )
             == ERROR_SUCCESS )
        {
            if ( DataType == REG_SZ )
            {
...
ACM::DriverProcedure( const HDRVR hdrvr, const UINT msg, LONG lParam1, LONG lParam2 )
{
    ...
    case ACMDM_STREAM_PREPARE:
        ...
        dwRes
            = OnStreamPrepareHeader( (LPACMDRVSTREAMINSTANCE)lParam1, (LPACMSTREAMHEADER)lParam2 );
        break;
    ...
    case ACMDM_STREAM_UNPREPARE:
        ...
        dwRes = OnStreamUnPrepareHeader( (LPACMDRVSTREAMINSTANCE)lParam1,
    ...
    default:
        ...
        my_debug.OutPut(
            DEBUG_LEVEL_MSG,
            "ACM::DriverProc unknown message (0x%08X), lParam1 = 0x%08X, lParam2 = 0x%08X",
            msg,
            lParam1,
            lParam2 );
...
*/
inline DWORD
ACM::OnFormatDetails( LPACMFORMATDETAILS a_FormatDetails, const LPARAM a_Query )
{
    ...
    case ACM_FORMATDETAILSF_INDEX:
        my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                         "enter ACM_FORMATDETAILSF_INDEX for index 0x%04X:%03d",
                         a_FormatDetails->dwFormatTag,
                         a_FormatDetails->dwFormatIndex );
        if ( a_FormatDetails->dwFormatTag == PERSONAL_FORMAT )
        {
            if ( a_FormatDetails->dwFormatIndex < GetNumberEncodingFormats( ) )
            {
                ...
                my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                                 "format in  : channels %d, sample rate %d",
                                 WaveExt->nChannels,
                                 WaveExt->nSamplesPerSec );
                GetMP3FormatForIndex(
                    a_FormatDetails->dwFormatIndex, *WaveExt, a_FormatDetails->szFormat );
                my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                                 "format out : channels %d, sample rate %d",
                                 WaveExt->nChannels,
                                 WaveExt->nSamplesPerSec );
            ...
            {
                my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                                 "ACM_FORMATDETAILSF_INDEX unknown index 0x%04X:%03d",
                                 a_FormatDetails->dwFormatTag,
                                 a_FormatDetails->dwFormatIndex );
            }
        }
        ...
        {
            my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                             "Unknown a_FormatDetails->dwFormatTag = 0x%08X",
                             a_FormatDetails->dwFormatTag );
        }
    ...
    case ACM_FORMATDETAILSF_FORMAT:
        ...
        my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                         "enter ACM_FORMATDETAILSF_FORMAT : 0x%04X:%03d, format in : channels %d, "
                         "sample rate %d",
                         a_FormatDetails->dwFormatTag,
                         a_FormatDetails->dwFormatIndex,
                         WaveExt->nChannels,
...
*/
inline DWORD
ACM::OnFormatTagDetails( LPACMFORMATTAGDETAILS a_FormatTagDetails, const LPARAM a_Query )
{
    DWORD Result;
    ...
    {
        my_debug.OutPut(
            DEBUG_LEVEL_FUNC_CODE, "ACMDM_FORMATTAG_DETAILS, a_Query = 0x%08X", a_Query );
        ...
        case ACM_FORMATTAGDETAILSF_INDEX:
            ...
            my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                             "get ACM_FORMATTAGDETAILSF_INDEX for index %03d",
            ...
            {
                my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                                 "ACM_FORMATTAGDETAILSF_INDEX for unsupported index %03d",
                                 a_FormatTagDetails->dwFormatTagIndex );
                Result = ACMERR_NOTPOSSIBLE;
            }
            break;
        ...
        case ACM_FORMATTAGDETAILSF_FORMATTAG:
            // Fill-in the informations corresponding to the a_FormatDetails->dwFormatTagIndex and
            // hdrvr given
        ...
        my_debug.OutPut(
            DEBUG_LEVEL_FUNC_CODE, "OnFormatTagDetails the_format = 0x%08X", the_format );
        switch ( the_format )
        {
        case WAVE_FORMAT_PCM:
        ...
        default:
            my_debug.OutPut(
                DEBUG_LEVEL_FUNC_CODE, "OnFormatTagDetails Unknown format 0x%08X", the_format );
            return ( ACMERR_NOTPOSSIBLE );
        }
        my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                         "OnFormatTagDetails %d possibilities for format 0x%08X",
                         a_FormatTagDetails->cStandardFormats,
                         the_format );
    }
    else
    {
        my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                         "a_FormatTagDetails->cbStruct < sizeof(*a_FormatDetails)" );
        Result = ACMERR_NOTPOSSIBLE;
    }
...
*/
inline DWORD
ACM::OnDriverDetails( const HDRVR hdrvr, LPACMDRIVERDETAILS a_DriverDetail )
{
    if ( my_hIcon == NULL )
    ...
    int u
        = MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, tmpStr, -1, a_DriverDetail->szLongName, 0 );
...
ACM::OnFormatSuggest( LPACMDRVFORMATSUGGEST a_FormatSuggest )
{
    ...
    my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                     "Suggest for source format = 0x%04X, channels = %d, Samples/s = %d, AvgB/s = "
                     "%d, BlockAlign = %d, b/sample = %d",
    ...
    my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                     "Suggested destination format = 0x%04X, channels = %d, Samples/s = %d, AvgB/s "
                     "= %d, BlockAlign = %d, b/sample = %d",
    ...
    case WAVE_FORMAT_PCM:
        ...
        {
            if ( a_FormatSuggest->pwfxSrc->nSamplesPerSec
                 != a_FormatSuggest->pwfxDst->nSamplesPerSec )
                return ( ACMERR_NOTPOSSIBLE );
        }
        else
        ...
        {
            if ( ( 16 != a_FormatSuggest->pwfxDst->wBitsPerSample )
                 && ( 8 != a_FormatSuggest->pwfxDst->wBitsPerSample ) )
                return ( ACMERR_NOTPOSSIBLE );
        }
        else
        {
    ...
    case PERSONAL_FORMAT:
        ...
        a_FormatSuggest->pwfxDst->nAvgBytesPerSec = a_FormatSuggest->pwfxDst->nSamplesPerSec
                                                    * a_FormatSuggest->pwfxDst->nChannels
...
*/
inline DWORD
ACM::OnStreamOpen( LPACMDRVSTREAMINSTANCE a_StreamInstance )
{
    DWORD Result = ACMERR_NOTPOSSIBLE;
    ...
    case WAVE_FORMAT_PCM:
        ...
        if ( a_StreamInstance->pwfxDst->wFormatTag == PERSONAL_FORMAT )
        {
            ...
            my_debug.OutPut(
                ...
                a_StreamInstance->pwfxDst->nSamplesPerSec,
                a_StreamInstance->pwfxDst->nChannels,
                a_StreamInstance->pwfxDst->wBitsPerSample,
                8 * a_StreamInstance->pwfxDst->nAvgBytesPerSec );

            /// \todo add the possibility to have channel resampling (mono to stereo / stereo to
            /// mono) \todo support resampling ? \todo only do the test on OutputFrequency in "Smart
            /// Output" mode
            ...
            else
            {
                if ( ( a_StreamInstance->fdwOpen & ACM_STREAMOPENF_QUERY ) == 0 )
                {
                    ...
                    if ( the_stream != NULL )
                    {
                        MPEGLAYER3WAVEFORMAT* casted
                            = (MPEGLAYER3WAVEFORMAT*)a_StreamInstance->pwfxDst;
    ...
    case PERSONAL_FORMAT:
        my_debug.OutPut(
            ...
            a_StreamInstance->pwfxSrc->nChannels,
            a_StreamInstance->pwfxSrc->wBitsPerSample,
            8 * a_StreamInstance->pwfxSrc->nAvgBytesPerSec );
        if ( a_StreamInstance->pwfxDst->wFormatTag == WAVE_FORMAT_PCM )
        {
            ...
            if ( ( a_StreamInstance->fdwOpen & ACM_STREAMOPENF_QUERY ) == 0 )
            {
                ...
                my_debug.OutPut(
                    DEBUG_LEVEL_FUNC_CODE,
                    "Open stream for PCM output (%05d samples %d channels %d bits/sample %d B/s)",
                    a_StreamInstance->pwfxDst->nSamplesPerSec,
                    a_StreamInstance->pwfxDst->nChannels,
                    a_StreamInstance->pwfxDst->wBitsPerSample,
                    a_StreamInstance->pwfxDst->nAvgBytesPerSec );
...
inline DWORD
ACM::OnStreamSize( LPACMDRVSTREAMINSTANCE a_StreamInstance, LPACMDRVSTREAMSIZE the_StreamSize )
{
    DWORD Result = ACMERR_NOTPOSSIBLE;
    ...
    case ACM_STREAMSIZEF_DESTINATION:
        my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                         "Get source buffer size for destination size = %d",
                         the_StreamSize->cbDstLength );
        break;
    case ACM_STREAMSIZEF_SOURCE:
        my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                         "Get destination buffer size for source size = %d",
             ...
             && PERSONAL_FORMAT == a_StreamInstance->pwfxDst->wFormatTag )
        {
            ...
            {
                the_StreamSize->cbDstLength
                    = the_stream->GetOutputSizeForInput( the_StreamSize->cbSrcLength );
                Result = MMSYSERR_NOERROR;
            }
        }
...
inline DWORD
ACM::OnStreamClose( LPACMDRVSTREAMINSTANCE a_StreamInstance )
{
    DWORD Result = ACMERR_NOTPOSSIBLE;

    my_debug.OutPut(
        DEBUG_LEVEL_FUNC_CODE, "OnStreamClose the stream 0x%X", a_StreamInstance->dwInstance );
    if ( WAVE_FORMAT_PCM == a_StreamInstance->pwfxSrc->wFormatTag
         && PERSONAL_FORMAT == a_StreamInstance->pwfxDst->wFormatTag )
    {
...
inline DWORD
ACM::OnStreamPrepareHeader( LPACMDRVSTREAMINSTANCE a_StreamInstance,
                            LPACMSTREAMHEADER a_StreamHeader )
{
    DWORD Result = ACMERR_NOTPOSSIBLE;

    my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                     "  prepare : Src : %d (0x%08X) / %d - Dst : %d (0x%08X) / %d",
...
inline DWORD
ACM::OnStreamUnPrepareHeader( LPACMDRVSTREAMINSTANCE a_StreamInstance,
                              LPACMSTREAMHEADER a_StreamHeader )
{
    DWORD Result = ACMERR_NOTPOSSIBLE;

    my_debug.OutPut( DEBUG_LEVEL_FUNC_CODE,
                     "unprepare : Src : %d / %d - Dst : %d / %d",
                     a_StreamHeader->cbSrcLength,
                     a_StreamHeader->cbSrcLengthUsed,
                     a_StreamHeader->cbDstLength,
                     a_StreamHeader->cbDstLengthUsed );
         ...
         && PERSONAL_FORMAT == a_StreamInstance->pwfxDst->wFormatTag )
    {
        ...
        if ( the_stream->close( a_StreamHeader->pbDst, &OutputSize )
             && ( OutputSize <= a_StreamHeader->cbDstLength ) )
        {
...
inline DWORD
ACM::OnStreamConvert( LPACMDRVSTREAMINSTANCE a_StreamInstance, LPACMDRVSTREAMHEADER a_StreamHeader )
{
    DWORD Result = ACMERR_NOTPOSSIBLE;
...
void
ACM::GetMP3FormatForIndex( const DWORD the_Index,
                           WAVEFORMATEX& the_Format,
                           unsigned short the_String[ ACMFORMATDETAILS_FORMAT_CHARS ] ) const
{
    ...
    if ( the_Index < bitrate_table.size( ) )
    {
        ...
        if ( bitrate_table[ the_Index ].mode == vbr_abr )
            wsprintfA( temp,
                       "%d Hz, %d kbps ABR, %s",
                       the_Format.nSamplesPerSec,
                       the_Format.nAvgBytesPerSec * 8 / 1000,
        ...
        MultiByteToWideChar(
            CP_ACP, MB_PRECOMPOSED, temp, -1, the_String, ACMFORMATDETAILS_FORMAT_CHARS );
    }
}

void
ACM::GetPCMFormatForIndex( const DWORD the_Index,
                           WAVEFORMATEX& the_Format,
                           unsigned short the_String[ ACMFORMATDETAILS_FORMAT_CHARS ] ) const
{
    the_Format.nChannels = SIZE_CHANNEL_MODE - int( the_Index % SIZE_CHANNEL_MODE );
    ...
    the_Format.nAvgBytesPerSec
        = the_Format.nSamplesPerSec * the_Format.nChannels * the_Format.wBitsPerSample / 8;
}

DWORD
ACM::GetNumberEncodingFormats( ) const
{
    return bitrate_table.size( );
}

bool
ACM::IsSmartOutput( const int frequency, const int bitrate, const int channels ) const
{
    double compression_ratio = double( frequency * 2 * channels ) / double( bitrate * 100 );

    // my_debug.OutPut(DEBUG_LEVEL_FUNC_DEBUG, "compression_ratio %f, freq %d, bitrate %d, channels
    // %d", compression_ratio, frequency, bitrate, channels);
