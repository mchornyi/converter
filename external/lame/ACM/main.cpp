...
{
    void*
    acm_Calloc( size_t num, size_t size )
    {
        return LocalAlloc( LPTR, num * size );  // VirtualAlloc
    }

    void*
    acm_Malloc( size_t size )
    {
...
DriverProc( DWORD dwDriverId, HDRVR hdrvr, UINT msg, LONG lParam1, LONG lParam2 )
{
    ...
    case DRV_OPEN:  // acmDriverOpen
    {
        ...
        if ( debug != NULL )
        {
            ...
            if ( lParam2 != NULL )
                debug->OutPut(
                    DEBUG_LEVEL_MSG, "DRV_OPEN (ID 0x%08X), pDesc = 0x%08X", dwDriverId, lParam2 );
            else
        ...
        if ( lParam2 != NULL )
        {
            ...
            if ( pDesc->fccType != ACMDRIVERDETAILS_FCCTYPE_AUDIOCODEC )
            {
                ...
                {
                    debug->OutPut(
                        DEBUG_LEVEL_FUNC_CODE, "wrong pDesc->fccType (0x%08X)", pDesc->fccType );
                }
                return NULL;
            }
        }
    ...
    case DRV_LOAD:
    {
        ...
        if ( debug != NULL )
        {
            //				debug->OutPut(DEBUG_LEVEL_MSG, "DRV_LOAD, version %s %s %s", ACM_VERSION,
            //__DATE__, __TIME__);
        ...
        else
        {
            if ( debug != NULL )
            {
                debug->OutPut( DEBUG_LEVEL_MSG,
                               "Driver not opened, unknown message (0x%08X), lParam1 = 0x%08X, "
                               "lParam2 = 0x%08X",
                               msg,
                               lParam1,
                               lParam2 );
