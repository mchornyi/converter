...
const char*
TiXmlBase::SkipWhiteSpace( const char* p )
{
    if ( !p || !*p )
    {
        return 0;
...
/*static*/ bool
TiXmlBase::StreamWhiteSpace( std::istream* in, std::string* tag )
{
    for ( ;; )
    {
        if ( !in->good( ) )
...
/*static*/ bool
TiXmlBase::StreamTo( std::istream* in, int character, std::string* tag )
{
    while ( in->good( ) )
    {
        int c = in->peek( );
...
const char*
TiXmlBase::ReadName( const char* p, string* name )
{
    *name = "";
    assert( p );
...
const char*
TiXmlBase::GetEntity( const char* p, char* value )
{
...
bool
TiXmlBase::StringEqual( const char* p, const char* tag, bool ignoreCase )
{
    assert( p );
    if ( !p || !*p )
    {
...
const char*
TiXmlBase::ReadText(
    const char* p, string* text, bool trimWhiteSpace, const char* endTag, bool caseInsensitive )
{
    *text = "";
...
void
TiXmlDocument::StreamIn( std::istream* in, std::string* tag )
{
...
const char*
TiXmlDocument::Parse( const char* p )
{
...
void
TiXmlElement::StreamIn( std::istream* in, std::string* tag )
{
...
const char*
TiXmlElement::Parse( const char* p )
{
    p = SkipWhiteSpace( p );
    ...
    while ( p && *p )
    {
        ...
        else if ( *p == '>' )
        {
            ...
            ++p;
            p = ReadValue( p );  // Note this is an Element method, and will set the error if one
                                 // happens.
            if ( !p || !*p )
                return 0;
...
const char*
TiXmlElement::ReadValue( const char* p )
{
    TiXmlDocument* document = GetDocument( );
...
void
TiXmlUnknown::StreamIn( std::istream* in, std::string* tag )
{
    while ( in->good( ) )
    {
        int c = in->get( );
...
const char*
TiXmlUnknown::Parse( const char* p )
{
    TiXmlDocument* document = GetDocument( );
    p = SkipWhiteSpace( p );
    if ( !p || !*p || *p != '<' )
...
void
TiXmlComment::StreamIn( std::istream* in, std::string* tag )
{
    while ( in->good( ) )
    {
        int c = in->get( );
...
const char*
TiXmlComment::Parse( const char* p )
{
    TiXmlDocument* document = GetDocument( );
    value = "";
...
const char*
TiXmlAttribute::Parse( const char* p )
{
    p = SkipWhiteSpace( p );
    if ( !p || !*p )
        return 0;
...
void
TiXmlText::StreamIn( std::istream* in, std::string* tag )
{
    while ( in->good( ) )
    {
        int c = in->peek( );
...
void
TiXmlDeclaration::StreamIn( std::istream* in, std::string* tag )
{
    while ( in->good( ) )
    {
        int c = in->get( );
...
const char*
TiXmlDeclaration::Parse( const char* p )
{
    p = SkipWhiteSpace( p );
