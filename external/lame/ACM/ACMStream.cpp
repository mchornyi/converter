...
const bool
ACMStream::Erase( const ACMStream* a_ACMStream )
{
    delete a_ACMStream;
    return true;
}
...
bool
ACMStream::init( const int nSamplesPerSec,
                 const int nOutputSamplesPerSec,
                 const int nChannels,
                 const int nAvgBytesPerSec,
                 const vbr_mode mode )
{
    bool bResult = false;
...
bool
ACMStream::open( const AEncodeProperties& the_Properties )
{
    bool bResult = false;
    ...
    if ( my_Channels == 1 )
        ...
        lame_set_mode(
            gfp,
            (MPEG_mode_e)the_Properties.GetChannelModeValue( ) );  /// \todo Get the mode from the
    ...
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "version                =%d", lame_get_version( gfp ) );
    ...
    my_debug->OutPut( DEBUG_LEVEL_FUNC_DEBUG,
                      "sampling frequency     =%.1f kHz",
                      lame_get_in_samplerate( gfp ) / 1000.0 );
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "bitrate                =%d kbps", lame_get_brate( gfp ) );
    my_debug->OutPut( DEBUG_LEVEL_FUNC_DEBUG,
    ...
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "Quality Setting        =%d", lame_get_quality( gfp ) );

    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "Low pass frequency     =%d", lame_get_lowpassfreq( gfp ) );
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "Low pass width         =%d", lame_get_lowpasswidth( gfp ) );

    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "High pass frequency    =%d", lame_get_highpassfreq( gfp ) );
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "High pass width        =%d", lame_get_highpasswidth( gfp ) );

    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "No Short Blocks        =%d", lame_get_no_short_blocks( gfp ) );

    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "de-emphasis            =%d", lame_get_emphasis( gfp ) );
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "private flag           =%d", lame_get_extension( gfp ) );

    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "copyright flag         =%d", lame_get_copyright( gfp ) );
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "original flag          =%d", lame_get_original( gfp ) );
    my_debug->OutPut( DEBUG_LEVEL_FUNC_DEBUG,
                      "CRC                    =%s",
    ...
    my_debug->OutPut(
        DEBUG_LEVEL_FUNC_DEBUG, "Disable Resorvoir      =%d", lame_get_disable_reservoir( gfp ) );
    my_debug->OutPut( DEBUG_LEVEL_FUNC_DEBUG,
...
bool
ACMStream::close( LPBYTE pOutputBuffer, DWORD* pOutputSize )
{
    bool bResult = false;
...
DWORD
ACMStream::GetOutputSizeForInput( const DWORD the_SrcLength ) const
{
...
bool
ACMStream::ConvertBuffer( LPACMDRVSTREAMHEADER a_StreamHeader )
{
    bool result;
    ...
    DWORD InSize = a_StreamHeader->cbSrcLength / 2,
          OutSize = a_StreamHeader->cbDstLength;  // 2 for 8<->16 bits
    ...
    {
        nOutputSamples = lame_encode_buffer( gfp,
                                             (PSHORT)a_StreamHeader->pbSrc,
                                             (PSHORT)a_StreamHeader->pbSrc,
                                             dwSamples,
    ...
    {
        nOutputSamples = lame_encode_buffer_interleaved( gfp,
                                                         (PSHORT)a_StreamHeader->pbSrc,
                                                         dwSamples,
                                                         a_StreamHeader->pbDst,
                                                         a_StreamHeader->cbDstLength );
    ...
    my_debug->OutPut( DEBUG_LEVEL_FUNC_CODE,
                      "UsedSize = %d / EncodedSize = %d, result = %d (%d <= %d)",
                      InSize,
                      OutSize,
                      result,
    ...
    {
        my_debug->OutPut( DEBUG_LEVEL_FUNC_DEBUG,
                          "ACMStream::ConvertBuffer result = %d (0x%02X 0x%02X)",
                          result,
                          a_StreamHeader->pbDst[ 0 ],
                          a_StreamHeader->pbDst[ 1 ] );
    }
...
unsigned int
ACMStream::GetOutputSampleRate( int samples_per_sec, int bitrate, int channels )
{
    if ( bitrate == 0 )
