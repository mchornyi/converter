...
#define _RELEASEDEBUG 0
    ...
    static void
    DebugPrintf( const char* pzFormat, ... )
    {
        char szBuffer[ 1024 ] = {
            '\0',
        };
        ...
        GetModuleFileNameA( gs_hModule, szFileName, sizeof( szFileName ) );
    ...
    static void
    PresetOptions( lame_global_flags* gfp, LONG myPreset )
    {
        switch ( myPreset )
        {
        /*-1*/ case LQP_NOPRESET:
        ...
        if ( (int)lameConfig.format.LHV1.nPreset < (int)LQP_STANDARD )
        {
            ...
            if ( lameConfig.format.LHV1.dwVbrAbr_bps > 0 )
            {
                ...
                lame_set_VBR_mean_bitrate_kbps(
                    gfp, ( lameConfig.format.LHV1.dwVbrAbr_bps + 500 ) / 1000 );
    ...
    __declspec( dllexport ) BE_ERR
        beFlushNoGap( HBE_STREAM hbeStream, PBYTE pOutput, PDWORD pdwOutput )
    {
    ...
    pbeVersion->byMMXEnabled = 0;
        ...
        // Set month
        if ( strcmp( lpszTemp, "Jan" ) == 0 )
            pbeVersion->byMonth = 1;
        if ( strcmp( lpszTemp, "Feb" ) == 0 )
            pbeVersion->byMonth = 2;
        if ( strcmp( lpszTemp, "Mar" ) == 0 )
            pbeVersion->byMonth = 3;
        if ( strcmp( lpszTemp, "Apr" ) == 0 )
            pbeVersion->byMonth = 4;
        if ( strcmp( lpszTemp, "May" ) == 0 )
            pbeVersion->byMonth = 5;
        if ( strcmp( lpszTemp, "Jun" ) == 0 )
            pbeVersion->byMonth = 6;
        if ( strcmp( lpszTemp, "Jul" ) == 0 )
            pbeVersion->byMonth = 7;
        if ( strcmp( lpszTemp, "Aug" ) == 0 )
            pbeVersion->byMonth = 8;
        if ( strcmp( lpszTemp, "Sep" ) == 0 )
            pbeVersion->byMonth = 9;
        if ( strcmp( lpszTemp, "Oct" ) == 0 )
            pbeVersion->byMonth = 10;
        if ( strcmp( lpszTemp, "Nov" ) == 0 )
            pbeVersion->byMonth = 11;
        if ( strcmp( lpszTemp, "Dec" ) == 0 )
            pbeVersion->byMonth = 12;
    ...
    __declspec( dllexport ) BE_ERR beEncodeChunk(
        HBE_STREAM hbeStream, DWORD nSamples, PSHORT pSamples, PBYTE pOutput, PDWORD pdwOutput )
    {
        // Encode it
        int dwSamples;
        int nOutputSamples = 0;
    ...
    DllMain( HANDLE hModule, DWORD ul_reason_for_call, LPVOID lpReserved )
    {
        ...
        case DLL_PROCESS_ATTACH:
            ...
            gs_bLogFile
                = GetPrivateProfileIntA( "Debug", "WriteLogFile", gs_bLogFile, "lame_enc.ini" );
            break;
    ...
    static void
    dump_config( lame_global_flags* gfp )
    {
