...
#define DMA_SAMPLES                                                                            \
    512 /* My Linux driver uses a DMA buffer of 65536*16 bit, which is 32768 samples in 16 bit \
           stereo mode */
...
int
fft( compl*fn, const size_t newlen )
{
    static size_t len = 0;
    static int bits = 0;
    size_t i;
...
void
printnumber( long double x )
{
    unsigned exp = 0;

    if ( x < 9.999995 )
        fprintf( stderr, "%7.5f", (double)x );
    else if ( x < 99.99995 )
        fprintf( stderr, "%7.4f", (double)x );
    else if ( x < 999.9995 )
        fprintf( stderr, "%7.3f", (double)x );
    else if ( x < 9999.995 )
        fprintf( stderr, "%7.2f", (double)x );
    else if ( x < 99999.95 )
        fprintf( stderr, "%7.1f", (double)x );
    else if ( x < 999999.5 )
        fprintf( stderr, "%6.0f.", (double)x );
    else if ( x < 9999999.5 )
        fprintf( stderr, "%7.0f", (double)x );
    else if ( x < 9.9995e9 )
    {
        while ( x >= 9.9995 )
            exp++, x /= 10;
...
void
analyze_stereo( const stereo_t* p1, const stereo_t* p2, size_t len, korr_t* const k )
{
...
int
feed( int fd, const stereo_t* p, int len )
{
    int i;
...
int
feed2( int fd, const stereo_t* p1, const stereo_t* p2, int len )
{
...
int
feedfac( int fd, const stereo_t* p1, const stereo_t* p2, int len, double fac1, double fac2 )
{
    stereo_t tmp[ 30000 ]; /* An arbitrary size, hope that no overruns occure */
    int i;
...
void
setup( int fdd, int samples, long freq )
{
    int status, org, arg;
...
void
Message( const char* s, size_t index, long freq, size_t start, size_t stop )
{
    ...
    fprintf( stderr,
             "\rListening %s  %2lu:%02lu.%02lu  "
             "(%1lu:%02lu.%02lu...%1lu:%02lu.%02lu)%*.*s\rListening %s",
             s,
             norm_index / 6000,
             norm_index / 100 % 60,
             norm_index % 100,
...
size_t
calc_true_index( size_t index, size_t start, size_t stop )
{
    if ( start >= stop )
        return start;
...
void
testing( const stereo_t* A, const stereo_t* B, size_t len, long freq )
{
    int c;
    ...
    while ( 1 )
    {
        ...
        case 4: /* A */
            strcpy( message, "A  " );
            if ( index + BF >= stop )
                index += feed( fd, A + index, stop - index ), state++;
            else
                index += feed( fd, A + index, BF );
            break;
        ...
        case 5: /* B */
            strcpy( message, "  B" );
            if ( index + BF >= stop )
                index += feed( fd, B + index, stop - index ), state--;
            else
                index += feed( fd, B + index, BF );
            break;
        ...
        case 6: /* X */
            strcpy( message, " X " );
            if ( index + BF >= stop )
                index += feed( fd, ( rnd ? B : A ) + index, stop - index ), state++;
            else
        ...
        case 7: /* !X */
            strcpy( message, "!X " );
            if ( index + BF >= stop )
                index += feed( fd, ( rnd ? A : B ) + index, stop - index ), state--;
            else
...
const decoder_t decoder[] = {
    ...
    {".mp2", PATH "mpg123 -w - %s" REDIR},  // MPEG Layer II        : www.iis.fhg.de,
                                            // www.uq.net.au/~zzmcheng, www.mpeg.org
    {".mp3", PATH "mpg123 -w - %s" REDIR},  // MPEG Layer III       : www.iis.fhg.de,
                                            // www.mp3dev.org, www.mpeg.org
    {".mp3pro", PATH "mpg123 -w - %s" REDIR},  // MPEG Layer III       : www.iis.fhg.de,
                                               // www.mp3dev.org, www.mpeg.org
    {".mpt", PATH "mpg123 -w - %s" REDIR},     // MPEG Layer III       : www.iis.fhg.de,
                                               // www.mp3dev.org, www.mpeg.org
    {".mpp", PATH "mppdec %s -" REDIR},        // MPEGplus             :
                                               // www.stud.uni-hannover.de/user/73884
    {".mpc", PATH "mppdec %s -" REDIR},        // MPEGplus             :
                                               // www.stud.uni-hannover.de/user/73884
    {".mp+", PATH "mppdec %s -" REDIR},        // MPEGplus             :
                                               // www.stud.uni-hannover.de/user/73884
    {".aac", PATH "faad -t.wav -w %s" REDIR},  // Advanced Audio Coding: psytel.hypermart.net,
                                               // www.aac-tech.com, sourceforge.net/projects/faac,
                                               // www.aac-audio.com, www.mpeg.org
    {"aac.lqt", PATH "faad -t.wav -w %s" REDIR},  // Advanced Audio Coding: psytel.hypermart.net,
                                                  // www.aac-tech.com,
    ...
    {".ogg",
     PATH "ogg123 -d wav -o file:" STDOUT " %s" REDIR},  // Ogg Vorbis           :
                                                         // www.xiph.org/ogg/vorbis/index.html
    {".pac", PATH "lpac -x %s " STDOUT REDIR},           // Lossless predictive Audio Compression:
                                                // www-ft.ee.tu-berlin.de/~liebchen/lpac.html
                                                // (liebchen@ft.ee.tu-berlin.de)
    {".shn", PATH "shorten -x < %s" REDIR},  // Shorten              : shnutils.freeshell.org,
                                             // www.softsound.com/Shorten.html
                                             // (shnutils@freeshell.org, shorten@softsound.com)
    ...
    {".raw", PATH "sox -r44100 -sw -c2 -traw %s -twav -sw -" REDIR},  // raw files are treated as CD
                                                                      // like audio
    {".cdr", PATH "sox -r44100 -sw -c2 -traw %s -twav -sw -" REDIR},  // CD-DA files are treated as
                                                                      // CD like audio, no
                                                                      // preemphasis info available
    {".rm", "echo %s '???'" REDIR},         // Real Audio           : www.real.com
    {".epc", "echo %s '???'" REDIR},        // ePAC                 : www.audioveda.com,
                                            // www.lucent.com/ldr
    {".mov", "echo %s '???'" REDIR},        // QDesign Music 2      : www.qdesign.com
    {".vqf", "echo %s '???'" REDIR},        // TwinVQ               :
                                            // www.yamaha-xg.com/english/xg/SoundVQ, www.vqf.com,
                                            // sound.splab.ecl.ntt.co.jp/twinvq-e
    ...
    {".ape",
     "( " PATH
     "MAC %s _._.wav -d > /dev/null; cat _._.wav; rm _._.wav )" REDIR},  // Monkey's Audio Codec :
                                                                         // www.monkeysaudio.com
                                                                         // (email@monkeysaudio.com)
    {".rka", "( " PATH "rkau %s _._.wav   > /dev/null; cat _._.wav; rm _._.wav )" REDIR},  // RK
                                                                                           // Audio:
    {".rkau",
     "( " PATH "rkau %s _._.wav   > /dev/null; cat _._.wav; rm _._.wav )" REDIR},  // RK Audio:
    {".mod",
     PATH
     "xmp -b16 -c -f44100 --stereo -o- %s | sox -r44100 -sw -c2 -traw - -twav -sw -" REDIR},  // Amiga's
                                                                                              // Music
                                                                                              // on
                                                                                              // Disk:
...
int
readwave( stereo_t* buff, size_t maxlen, const char* name, size_t* len )
{
...
double
cross_analyze( const stereo_t* p1, const stereo_t* p2, size_t len )
{
    float P1[ MAX ][ 2 ];
    float P2[ MAX ][ 2 ];
    int i;
...
void
DC_cancel( stereo_t* p, size_t len )
{
    double sum1 = 0;
    double sum2 = 0;
    size_t i;
...
void
multiply( char c, stereo_t* p, size_t len, double fact )
{
    size_t i;
...
main( int argc, char** argv )
{
    ...
    if ( verbose )
    {
        ...
        fprintf(
            stderr,
            "Delay Ch2 is %.4f samples\n",
            cross_analyze(
                (stereo_t*)( ( (sample_t*)A ) + 1 ), (stereo_t*)( ( (sample_t*)B ) + 1 ), len ) );
    }
