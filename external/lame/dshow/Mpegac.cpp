...
#define GET_FRAMELENGTH( bitrate, sample_rate ) \
    ( ( WORD )( ( ( sample_rate < 32000 ? 72000 : 144000 ) * ( bitrate ) ) / ( sample_rate ) ) )
...
output_caps_t OutputCapabilities[] = {  // {SampleRate, BitRate}
    {48000, 320},
    {48000, 256},
    {48000, 224},
...
AMOVIESETUP_MEDIATYPE sudMpgInputType[] = {{&MEDIATYPE_Audio, &MEDIASUBTYPE_PCM}};
AMOVIESETUP_MEDIATYPE sudMpgOutputType[] = {{&MEDIATYPE_Audio, &MEDIASUBTYPE_MPEG1AudioPayload},
                                            {&MEDIATYPE_Audio, &MEDIASUBTYPE_MPEG2_AUDIO},
                                            {&MEDIATYPE_Audio, &MEDIASUBTYPE_MP3},
                                            {&MEDIATYPE_Stream, &MEDIASUBTYPE_MPEG1Audio}};

AMOVIESETUP_PIN sudMpgPins[] = {{L"PCM Input",
                                 FALSE,                       // bRendered
                                 FALSE,                       // bOutput
                                 FALSE,                       // bZero
                                 FALSE,                       // bMany
...
AMOVIESETUP_FILTER sudMpgAEnc = {&CLSID_LAMEDShowFilter,
...
CFactoryTemplate g_Templates[]
    = {{g_wszName, &CLSID_LAMEDShowFilter, CMpegAudEnc::CreateInstance, NULL, &sudMpgAEnc},
       {L"LAME Audio Encoder Property Page",
        &CLSID_LAMEDShow_PropertyPage,
        CMpegAudEncPropertyPage::CreateInstance},
       {L"LAME Audio Encoder Property Page",
        &CLSID_LAMEDShow_PropertyPageAdv,
        CMpegAudEncPropertyPageAdv::CreateInstance},
...
REGFILTER2 rf2FilterReg = {
    1,                     // Version number.
    DEFAULT_FILTER_MERIT,  // Merit. This should match the merit specified in the AMOVIESETUP_FILTER
                           // definition
...
DllRegisterServer( void )
{
    ...
    hr = CoCreateInstance(
        CLSID_FilterMapper2, NULL, CLSCTX_INPROC_SERVER, IID_IFilterMapper2, (void**)&pFM2 );
    if ( SUCCEEDED( hr ) )
    {
        hr = pFM2->RegisterFilter( CLSID_LAMEDShowFilter,           // Filter CLSID.
...
DllUnregisterServer( )
{
    ...
    if ( SUCCEEDED( hr ) )
    {
        hr = pFM2->UnregisterFilter(
            &CLSID_AudioCompressorCategory, g_wszName, CLSID_LAMEDShowFilter );
        pFM2->Release( );
    }
    return hr;
}
...
CUnknown*
CMpegAudEnc::CreateInstance( LPUNKNOWN lpunk, HRESULT* phr )
{
...
CMpegAudEnc::GetSetupData( )
{
    return &sudMpgAEnc;
}

HRESULT
CMpegAudEnc::Receive( IMediaSample* pSample )
{
    CAutoLock lock( &m_cs );
    ...
    if ( sample_size <= 0 || pSourceBuffer == NULL || m_hasFinished
         || ( gotValidTime && rtStart < 0 ) )
        return S_OK;
...
HRESULT
CMpegAudEnc::FlushEncodedSamples( )
{
...
CMpegAudEnc::StartStreaming( )
{
    ...
    {
        dwOutSampleRate
            = ( (WAVEFORMATEX*)m_pOutput->CurrentMediaType( ).Format( ) )->nSamplesPerSec;
    }
...
CMpegAudEnc::StopStreaming( )
{
    ...
    if ( m_bStreamOutput && m_pOutput->IsConnected( ) != FALSE )
    {
        ...
        if ( pDwnstrmInputPin
             && FAILED( pDwnstrmInputPin->QueryInterface( IID_IStream, (LPVOID*)( &pStream ) ) ) )
        {
            pStream = NULL;
        }
    }
...
CMpegAudEnc::BeginFlush( )
{
    ...
    if ( SUCCEEDED( hr ) )
    {
        ...
        if ( m_bStreamOutput && m_pOutput->IsConnected( ) != FALSE )
        {
            ...
            if ( pDwnstrmInputPin
                 && SUCCEEDED(
                        pDwnstrmInputPin->QueryInterface( IID_IStream, (LPVOID*)( &pStream ) ) ) )
            {
                ULARGE_INTEGER size;
                size.QuadPart = m_rtBytePos;
                pStream->SetSize( size );
...
////////////////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEnc::SetMediaType( PIN_DIRECTION direction, const CMediaType* pmt )
{
    HRESULT hr = S_OK;
    ...
    if ( direction == PINDIR_INPUT )
    {
        ...
        if ( pwfx )
            m_bytesToDuration
                = (float)1.e7 / (float)( pwfx->nChannels * sizeof( short ) * pwfx->nSamplesPerSec );
        else
            m_bytesToDuration = 0.0;

        // Parse the encoder output capabilities into the subset of capabilities that are supported
        // for the current input format. This listing will be utilized by the IAMStreamConfig
        // Interface.
    ...
    else if ( direction == PINDIR_OUTPUT )
    {
        ...
        if ( m_pInput && m_pInput->IsConnected( ) )
        {
            ...
            if ( FAILED( hr ) )
            {
                ...
                // Note: The CheckMediaType method has already called QueryAccept on the upstream
                // filter.
...
////////////////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEnc::CheckInputType( const CMediaType* mtIn )
{
...
////////////////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEnc::CheckTransform( const CMediaType* mtIn, const CMediaType* mtOut )
{
...
////////////////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEnc::DecideBufferSize( IMemAllocator* pAllocator, ALLOCATOR_PROPERTIES* pProperties )
{
    HRESULT hr = S_OK;
    ...
    if ( pProperties->cBuffers == 0 )
        pProperties->cBuffers
            = 1;  // If downstream filter didn't suggest a buffer count then default to 1
...
////////////////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEnc::GetMediaType( int iPosition, CMediaType* pMediaType )
{
...
CMpegAudEnc::Reconnect( )
{
    ...
    if ( m_pOutput && m_pOutput->IsConnected( ) && m_State == State_Stopped )
    {
        ...
        if ( ( hr = m_Encoder.SetOutputType( mec ) ) == S_OK )
        {
            ...
            if ( m_pOutput->CurrentMediaType( ) == cmt )
                return S_OK;
...
////////////////////////////////////////////////////////////////////////////
void
CMpegAudEnc::LoadOutputCapabilities( DWORD sample_rate )
{
    m_CapsNum = 0;
...
//
void
CMpegAudEnc::ReadPresetSettings( MPEG_ENCODER_CONFIG* pmec )
{
...
////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEnc::GetPages( CAUUID* pcauuid )
{
    GUID* pguid;
...
STDMETHODIMP
CMpegAudEnc::NonDelegatingQueryInterface( REFIID riid, void** ppv )
{
...
//
STDMETHODIMP
CMpegAudEnc::get_PESOutputEnabled( DWORD* dwEnabled )
{
...
STDMETHODIMP
CMpegAudEnc::set_PESOutputEnabled( DWORD dwEnabled )
{
...
STDMETHODIMP
CMpegAudEnc::get_MPEGLayer( DWORD* dwLayer )
{
    MPEG_ENCODER_CONFIG mec;
...
STDMETHODIMP
CMpegAudEnc::set_MPEGLayer( DWORD dwLayer )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
    if ( dwLayer == 2 )
...
STDMETHODIMP
CMpegAudEnc::get_Bitrate( DWORD* dwBitrate )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_Bitrate( DWORD dwBitrate )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_Variable( DWORD* dwVariable )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_Variable( DWORD dwVariable )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_VariableMin( DWORD* dwMin )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_VariableMin( DWORD dwMin )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_VariableMax( DWORD* dwMax )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_VariableMax( DWORD dwMax )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_Quality( DWORD* dwQuality )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_Quality( DWORD dwQuality )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_VariableQ( DWORD* dwVBRq )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_VariableQ( DWORD dwVBRq )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
    mec.dwVBRq = dwVBRq;
...
STDMETHODIMP
CMpegAudEnc::get_SourceSampleRate( DWORD* dwSampleRate )
{
    *dwSampleRate = 0;
...
STDMETHODIMP
CMpegAudEnc::get_SourceChannels( DWORD* dwChannels )
{
    WAVEFORMATEX wf;
    if ( FAILED( m_Encoder.GetInputType( &wf ) ) )
        return E_FAIL;
...
STDMETHODIMP
CMpegAudEnc::get_SampleRate( DWORD* dwSampleRate )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_SampleRate( DWORD dwSampleRate )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_ChannelMode( DWORD* dwChannelMode )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_ChannelMode( DWORD dwChannelMode )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_ForceMS( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_ForceMS( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_CRCFlag( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_ForceMono( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_SetDuration( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_SampleOverlap( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_CRCFlag( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_ForceMono( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_SetDuration( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_SampleOverlap( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_EnforceVBRmin( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_EnforceVBRmin( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_VoiceMode( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_VoiceMode( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_KeepAllFreq( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_KeepAllFreq( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_StrictISO( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_StrictISO( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_NoShortBlock( DWORD* dwNoShortBlock )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_NoShortBlock( DWORD dwNoShortBlock )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_XingTag( DWORD* dwXingTag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_XingTag( DWORD dwXingTag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_OriginalFlag( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_OriginalFlag( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_CopyrightFlag( DWORD* dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_CopyrightFlag( DWORD dwFlag )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_ModeFixed( DWORD* dwModeFixed )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::set_ModeFixed( DWORD dwModeFixed )
{
    MPEG_ENCODER_CONFIG mec;
    m_Encoder.GetOutputType( &mec );
...
STDMETHODIMP
CMpegAudEnc::get_ParameterBlockSize( BYTE* pcBlock, DWORD* pdwSize )
{
...
STDMETHODIMP
CMpegAudEnc::DefaultAudioEncoderProperties( )
{
...
STDMETHODIMP
CMpegAudEnc::LoadAudioEncoderPropertiesFromRegistry( )
{
...
STDMETHODIMP
CMpegAudEnc::SaveAudioEncoderPropertiesToRegistry( )
{
...
STDMETHODIMP
CMpegAudEnc::GetClassID( CLSID* pClsid )
{
...
HRESULT
CMpegAudEnc::WriteToStream( IStream* pStream )
{
...
// are in
HRESULT
CMpegAudEnc::ReadFromStream( IStream* pStream )
{
    MPEG_ENCODER_CONFIG mec;
...
STDMETHODIMP
CMpegAudEncOutPin::NonDelegatingQueryInterface( REFIID riid, void** ppv )
{
...
//////////////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEncOutPin::SetMediaType( const CMediaType* pmt )
{
    ...
    else
    {
        ...
        if ( pfmt->wfx.nChannels == 1 )
        {
            mec.ChMode = MONO;
        }
        else if ( pfmt->wfx.nChannels == 2 && mec.ChMode == MONO && !mec.bForceMono )
        {
            mec.ChMode = STEREO;
        }
    }
...
//////////////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEncOutPin::GetMediaType( int iPosition, CMediaType* pmt )
{
    if ( iPosition < 0 )
        return E_INVALIDARG;
    ...
    switch ( wf.nChannels )  // Determine if we need to alter ChMode based upon the channel count
                             // and ForceMono flag
    {
    case 1:
    {
    ...
    case 2:
    {
        if ( mec.ChMode == MONO && !mec.bForceMono )
        {
            m_CurrentOutputFormat.ChMode = STEREO;
        }
        else if ( mec.bForceMono )
        {
            m_CurrentOutputFormat.ChMode = MONO;
        }
        break;
    ...
    m_CurrentOutputFormat.nBitRate
        = ( mec.vmVariable == vbr_off ) ? mec.dwBitrate : mec.dwVariableMin;

    if ( pmt->majortype == MEDIATYPE_Stream )
        return NOERROR;  // No further config required for MEDIATYPE_Stream

    // Now configure the remainder of the WAVE_FORMAT_MPEGLAYER3 format block
    // and its parent AM_MEDIA_TYPE structure
    DECLARE_PTR( MPEGLAYER3WAVEFORMAT,
                 p_mp3wvfmt,
                 pmt->AllocFormatBuffer( sizeof( MPEGLAYER3WAVEFORMAT ) ) );
    ...
    p_mp3wvfmt->nBlockSize
        = GET_FRAMELENGTH( m_CurrentOutputFormat.nBitRate, p_mp3wvfmt->wfx.nSamplesPerSec );
...
//////////////////////////////////////////////////////////////////////////
HRESULT
CMpegAudEncOutPin::CheckMediaType( const CMediaType* pmtOut )
{
    ...
    // filter cannot use.
    if ( pmtOut->majortype != MEDIATYPE_Audio && pmtOut->majortype != MEDIATYPE_Stream )
    {
        return S_FALSE;
    }
...
CMpegAudEncOutPin::SetFormat( AM_MEDIA_TYPE* pmt )
{
...
HRESULT STDMETHODCALLTYPE
CMpegAudEncOutPin::GetFormat( AM_MEDIA_TYPE** ppmt )
{
...
HRESULT STDMETHODCALLTYPE
CMpegAudEncOutPin::GetNumberOfCapabilities( int* piCount, int* piSize )
{
    // The set of possible output formats depends on the input format,
    // so if the input pin is not connected, return a failure code.
    if ( !m_pFilter->m_pInput->IsConnected( ) )
    ...
    if ( mec.vmVariable != vbr_off )
    {
        *piCount = 0;
    }
    else
    {
        *piCount = m_pFilter->m_CapsNum;
    }
...
HRESULT STDMETHODCALLTYPE
CMpegAudEncOutPin::GetStreamCaps( int iIndex, AM_MEDIA_TYPE** pmt, BYTE* pSCC )
{
    // The set of possible output formats depends on the input format,
    // so if the input pin is not connected, return a failure code.
    if ( !m_pFilter->m_pInput->IsConnected( ) )
