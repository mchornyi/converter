...
//////////////////////////////////////////////////////////////////////
HRESULT
CEncoder::SetInputType( LPWAVEFORMATEX lpwfex, bool bJustCheck )
{
    CAutoLock l( &m_lock );
    ...
    if ( lpwfex->wFormatTag == WAVE_FORMAT_PCM )
    {
        ...
        {
            if ( lpwfex->nSamplesPerSec == 48000 || lpwfex->nSamplesPerSec == 44100
                 || lpwfex->nSamplesPerSec == 32000 || lpwfex->nSamplesPerSec == 24000
                 || lpwfex->nSamplesPerSec == 22050 || lpwfex->nSamplesPerSec == 16000
                 || lpwfex->nSamplesPerSec == 12000 || lpwfex->nSamplesPerSec == 11025
...
//////////////////////////////////////////////////////////////////////
HRESULT
CEncoder::SetOutputType( MPEG_ENCODER_CONFIG& mabsi )
{
    CAutoLock l( &m_lock );
...
//////////////////////////////////////////////////////////////////////
HRESULT
CEncoder::SetDefaultOutputType( LPWAVEFORMATEX lpwfex )
{
    CAutoLock l( &m_lock );
...
CEncoder::Init( )
{
    ...
    if ( !pgf )
    {
        ...
        if ( pgf = lame_init( ) )
        {
            ...
            {
                // int act_br = pgf->VBR ? pgf->VBR_min_bitrate_kbps + pgf->VBR_max_bitrate_kbps / 2
                // : pgf->brate;
            ...
            // encoder delay compensation
            {
                ...
                if ( nch == 2 )
                    out_bytes = lame_encode_buffer_interleaved(
                        pgf, start_padd, 48, m_outFrameBuf, OUT_BUFFER_SIZE );
                else
                    out_bytes = lame_encode_buffer(
                        pgf, start_padd, start_padd, 48, m_outFrameBuf, OUT_BUFFER_SIZE );
...
//////////////////////////////////////////////////////////////////////
HRESULT
CEncoder::Close( IStream* pStream )
{
    CAutoLock l( &m_lock );
    if ( pgf )
    {
...
//////////////////////////////////////////////////////////////////////
int
CEncoder::Encode( const short* pdata, int data_size )
{
    CAutoLock l( &m_lock );

    if ( !pgf || !m_outFrameBuf || !pdata || data_size < 0
    ...
    if ( m_outReadOffset > 0 )
    {
        if ( m_outOffset > m_outReadOffset )
            memmove(
                m_outFrameBuf, m_outFrameBuf + m_outReadOffset, m_outOffset - m_outReadOffset );
    ...
    while ( 1 )
    {
        ...
        if ( nch == 2 )
            ...
            out_bytes = lame_encode_buffer( pgf,
...
CEncoder::Finish( )
{
    ...
    m_outOffset
        += lame_encode_flush( pgf, m_outFrameBuf + m_outOffset, OUT_BUFFER_SIZE - m_outOffset );
...
int
getFrameLength( const unsigned char* pdata )
{
    if ( !pdata || pdata[ 0 ] != 0xff || ( pdata[ 1 ] & 0xe0 ) != 0xe0 )
        return -1;

    const int sample_rate_tab[ 4 ][ 4 ] = {
        {11025, 12000, 8000, 1}, {1, 1, 1, 1}, {22050, 24000, 16000, 1}, {44100, 48000, 32000, 1}};
...
#define EMPHASIS_RESERVED 2
    ...
    if ( version_id != MPEG_VERSION_RESERVED && layer == LAYER_III && bitrate_id != BITRATE_FREE
         && bitrate_id != BITRATE_RESERVED && sample_rate_id != SRATE_RESERVED
...
int
CEncoder::GetFrame( const unsigned char** pframe )
{
...
////////////////////////////////////////////////////////////////////////////////
int
CEncoder::GetBlockAligned( const unsigned char** pblock, int* piBufferSize, const long& cbAlign )
{
    ASSERT( piBufferSize );
...
HRESULT
CEncoder::maybeSyncWord( IStream* pStream )
{
    HRESULT hr = S_OK;
...
HRESULT
CEncoder::skipId3v2( IStream* pStream, size_t lametag_frame_size )
{
    HRESULT hr = S_OK;
    ULONG nbytes;
...
// Updates VBR tag
HRESULT
CEncoder::updateLameTagFrame( IStream* pStream )
{
    HRESULT hr = S_OK;
