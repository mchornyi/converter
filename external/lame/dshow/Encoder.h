...
public:
    ...
    HRESULT SetInputType( LPWAVEFORMATEX lpwfex,
                          bool bJustCheck = FALSE );  // returns E_INVALIDARG if not supported
    // GetInputType - returns current input type
    HRESULT
    GetInputType( WAVEFORMATEX* pwfex )
    {
        if ( m_bInpuTypeSet )
        {
    ...
    HRESULT SetOutputType( MPEG_ENCODER_CONFIG& mabsi );  // returns E_INVALIDARG if not supported
                                                          // or not compatible with input type
    // Return current MPEG audio settings
    HRESULT
    GetOutputType( MPEG_ENCODER_CONFIG* pmabsi )
    {
        if ( m_bOutpuTypeSet )
        {
