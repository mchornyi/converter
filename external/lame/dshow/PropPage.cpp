...
const char* szBitRateString[ 2 ][ 14 ] = {{"32 kbps",
                                           "40 kbps",
                                           "48 kbps",
                                           "56 kbps",
                                           "64 kbps",
...
LPCSTR szQualityDesc[ 10 ]
    = {"High", "High", "High", "High", "High", "Medium", "Medium", "Low", "Low", "Fast mode"};

LPCSTR szVBRqDesc[ 10 ] = {"0 - ~1:4",
                           "1 - ~1:5",
                           "2 - ~1:6",
                           "3 - ~1:7",
                           "4 - ~1:9",
...
////////////////////////////////////////////////////////////////
CUnknown*
CMpegAudEncPropertyPage::CreateInstance( LPUNKNOWN punk, HRESULT* phr )
{
    CMpegAudEncPropertyPage* pNewObject = new CMpegAudEncPropertyPage( punk, phr );
...
CMpegAudEncPropertyPage::CMpegAudEncPropertyPage( LPUNKNOWN punk, HRESULT* phr )
    : CBasePropertyPage(
          NAME( "Encoder Property Page" ), punk, IDD_AUDIOENCPROPS, IDS_AUDIO_PROPS_TITLE )
    , m_pAEProps( NULL )
{
    ASSERT( phr );
...
HRESULT
CMpegAudEncPropertyPage::OnConnect( IUnknown* pUnknown )
{
...
HRESULT
CMpegAudEncPropertyPage::OnDisconnect( )
{
    // Release the interface
    if ( m_pAEProps == NULL )
        return E_UNEXPECTED;
...
HRESULT
CMpegAudEncPropertyPage::OnActivate( void )
{
...
HRESULT
CMpegAudEncPropertyPage::OnDeactivate( void )
{
    return NOERROR;
}
...
////////////////////////////////////////////////////////////////
BOOL
CMpegAudEncPropertyPage::OnReceiveMessage( HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    switch ( uMsg )
    {
    case WM_HSCROLL:
    ...
    case WM_COMMAND:
        ...
        case IDC_COMBO_VBRMIN:
            ...
            {
                int nVariableMin
                    = SendDlgItemMessage( hwnd, IDC_COMBO_VBRMIN, CB_GETCURSEL, 0, 0L );
                DWORD dwSampleRate;
                m_pAEProps->get_SampleRate( &dwSampleRate );
                DWORD dwMin;
        ...
        case IDC_COMBO_VBRMAX:
            ...
            {
                int nVariableMax
                    = SendDlgItemMessage( hwnd, IDC_COMBO_VBRMAX, CB_GETCURSEL, 0, 0L );
                DWORD dwSampleRate;
                m_pAEProps->get_SampleRate( &dwSampleRate );
                DWORD dwMax;
        ...
        case IDC_COMBO_SAMPLE_RATE:
            ...
            {
                int nSampleRate
                    = SendDlgItemMessage( hwnd, IDC_COMBO_SAMPLE_RATE, CB_GETCURSEL, 0, 0L );
...
//
HRESULT
CMpegAudEncPropertyPage::OnApplyChanges( )
{
    m_pAEProps->get_Bitrate( &m_dwBitrate );
    m_pAEProps->get_Variable( &m_dwVariable );
    m_pAEProps->get_VariableMin( &m_dwMin );
...
//
void
CMpegAudEncPropertyPage::InitPropertiesDialog( HWND hwndParent )
{
    ...
    for ( k = 0; k < 10; k++ )
        SendDlgItemMessage(
            hwndParent, IDC_COMBO_VBRq, CB_ADDSTRING, 0, ( LPARAM )(LPCTSTR)szVBRqDesc[ k ] );
    DWORD dwVBRq;
    ...
    if ( dwSampleRate >= 32000 )
    {
        ...
        for ( int i = 0; i < 14; i++ )
            SendDlgItemMessage( hwndParent,
                                ...
                                CB_ADDSTRING,
                                0,
                                ( LPARAM )(LPCTSTR)szBitRateString[ 0 ][ i ] );
    }
    else
    {
        ...
        for ( int i = 0; i < 14; i++ )
            SendDlgItemMessage( hwndParent,
                                ...
                                CB_ADDSTRING,
                                0,
                                ( LPARAM )(LPCTSTR)szBitRateString[ 1 ][ i ] );
    }
    ...
    if ( dwSampleRate >= 32000 )
    {
        ...
        for ( j = 0; j < 14; j++ )
        {
            SendDlgItemMessage( hwndParent,
                                ...
                                CB_ADDSTRING,
                                0,
                                ( LPARAM )(LPCTSTR)szBitRateString[ 0 ][ j ] );
            SendDlgItemMessage( hwndParent,
                                IDC_COMBO_VBRMAX,
                                CB_ADDSTRING,
                                0,
    ...
    else
    {
        ...
        for ( j = 0; j < 14; j++ )
        {
            SendDlgItemMessage( hwndParent,
                                ...
                                CB_ADDSTRING,
                                0,
                                ( LPARAM )(LPCTSTR)szBitRateString[ 1 ][ j ] );
            SendDlgItemMessage( hwndParent,
                                IDC_COMBO_VBRMAX,
                                CB_ADDSTRING,
                                0,
...
////////////////////////////////////////////////////////////////
void
CMpegAudEncPropertyPage::EnableControls( HWND hwndParent, bool bEnable )
{
...
void
CMpegAudEncPropertyPage::SetDirty( )
{
    m_bDirty = TRUE;
    if ( m_pPageSite )
