...
gtkmakeframe( void )
{
    ...
    else
    {
        ...
        while ( lame_get_frameNum( gfp ) == pinfo->frameNum )
        {
            ...
            mp3count = lame_encode_buffer(
                gfp, Buffer[ 0 ], Buffer[ 1 ], iread, mp3buffer, sizeof( mp3buffer ) );
        ...
        mp3out = hip_decode1(
            hip, mp3buffer, mp3count, mpg123pcm[ 0 ], mpg123pcm[ 1 ] ); /* re-synthesis to pcm */
...
plot_frame( void )
{
    ...
    sprintf( title2,
             "%s  mask_ratio=%3.2f  %3.2f  ener_ratio=%3.2f  %3.2f",
             title,
             pplot->ms_ratio[ 0 ],
             pplot->ms_ratio[ 1 ],
             pplot->ms_ener_ratio[ 0 ],
             pplot->ms_ener_ratio[ 1 ] );
    ...
    for ( j = 1152 - n, i = 0; i < n; i++, j++ )
    {
        xcord[ i ] = i;
        if ( gtkinfo.msflag )
            ycord[ i ] = ch ? .5 * ( pplot1->pcmdata2[ 0 ][ j ] - pplot1->pcmdata2[ 1 ][ j ] )
    ...
    for ( i = 0; i < n; i++ )
    {
        ...
        if ( gtkinfo.msflag )
            ycord[ i + 224 ]
                = ch ? .5 * ( pplot2->pcmdata2[ 0 ][ i ] - pplot2->pcmdata2[ 1 ][ i ] )
    ...
    for ( gr = 0; gr < mode_gr; gr++ )
    {
        ...
        gpk_bargraph_draw(
            mdctbox[ gr ], 0, xcord, ycord, xmn, ymn, xmx, ymx, 1, title2, 0, barcolor );
        ...
        ymx = 11;
        gpk_bargraph_draw(
            mdctbox[ gr ], n, xcord, ycord, xmn, ymn, xmx, ymx, 0, title2, 0, barcolor );
    }
    ...
    if ( gtkinfo.kbflag )
    {
        for ( gr = 0; gr < mode_gr; gr++ )
        {
            ...
            xmx = n;
            gpk_bargraph_draw(
                enerbox[ gr ], n, xcord, ycord, xmn, ymn, xmx, ymx, 1, title2, 0, barcolor );
        }
    }
    else
    {
        ...
        for ( gr = 0; gr < mode_gr; gr++ )
        {
            ...
            gpk_bargraph_draw(
                enerbox[ gr ], n, xcord, ycord, xmn, ymn, xmx, ymx, 1, title2, barthick, barcolor );
            ...
            gpk_bargraph_draw(
                enerbox[ gr ], n, xcord, ycord, xmn, ymn, xmx, ymx, 0, title2, barthick, &red );
        }
    }
    ...
    for ( gr = 0; gr < mode_gr; gr++ )
    {
        ...
        xmx = n + 1;
        gpk_bargraph_draw(
            sfbbox[ gr ], n, xcord, ycord, xmn, ymn, xmx, ymx, 1, label2, 0, grcolor[ gr ] );
        ...
        else
        {
            ...
            else
            {
                ...
                if ( pinfo->frameNum > 0 ) /* start averaging at second frame */
                    gtkinfo.avebits
                        = ( gtkinfo.avebits * ( ( pinfo->frameNum ) - 1 ) + pinfo->totbits )
                          / ( pinfo->frameNum );
...
get_mp3x_version( void )
{
    ...
    static /*@observer@ */ const char* const str = XSTR( MP3X_MAJOR_VERSION ) "." XSTR(
        MP3X_MINOR_VERSION ) " (alpha " XSTR( MP3X_ALPHA_VERSION ) ", " __DATE__ " " __TIME__ ")";
#elif MP3X_BETA_VERSION > 0
    static /*@observer@ */ const char* const str = XSTR( MP3X_MAJOR_VERSION ) "." XSTR(
        MP3X_MINOR_VERSION ) " (beta " XSTR( MP3X_BETA_VERSION ) ", " __DATE__ ")";
#else
...
get_main_menu( GtkWidget* windows, GtkWidget** menubar )
{
    ...
    gtk_item_factory_create_items(
        item_factory, nmenu_items, (GtkItemFactoryEntry*)menu_items, NULL );
...
gtkcontrol( lame_global_flags* gfp2, char* inPath )
{
    ...
    gtkinfo.chflag = 0;
    gtkinfo.kbflag = 0;
    gtkinfo.flag123 = is_mpeg_file_format( global_reader.input_format ) ? 1 : 0; /* MP3 file=use
                                                                                    mpg123 output */
    gtkinfo.pupdate = 0;
    gtkinfo.avebits = 0;
    gtkinfo.maxbits = 0;
    ...
    adj = (GtkAdjustment*)gtk_adjustment_new(
        0, 0, (gint)lame_get_totalframes( gfp ) - 1, 0, 0, 0 );
    ...
    gtk_signal_connect(
        GTK_OBJECT( button ), "clicked", GTK_SIGNAL_FUNC( frameadv ), ( gpointer ) "-1" );
    ...
    gtk_signal_connect(
        GTK_OBJECT( button ), "clicked", GTK_SIGNAL_FUNC( frameadv ), ( gpointer ) "10" );
    gtk_box_pack_start( GTK_BOX( box1 ), button, TRUE, TRUE, 0 );
    gtk_widget_show( button );

    button = gtk_button_new_with_label( "+100" );
    gtk_signal_connect(
        GTK_OBJECT( button ), "clicked", GTK_SIGNAL_FUNC( frameadv ), ( gpointer ) "100" );
    gtk_box_pack_start( GTK_BOX( box1 ), button, TRUE, TRUE, 0 );
    gtk_widget_show( button );
