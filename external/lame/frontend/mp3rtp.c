 ...
 *
 *  gcc -I..\include -I..\libmp3lame -o mp3rtp mp3rtp.c ../libmp3lame/libmp3lame.a lametime.c
 * get_audio.c ieeefloat.c timestatus.c parse.c rtp.c -lm
 *
 *  wavrec -t 14400 -s 44100 -S /proc/self/fd/1 | ./mp3rtp 10.1.1.42 -V2 -b128 -B256 -
 * my_mp3file.mp3
 */
...
lame_main( lame_t gf, int argc, char** argv )
{
    ...
    if ( argc <= 2 )
    {
        console_printf(
            ...
            "    examples:\n"
            "      arecord -b 16 -s 22050 -w | ./mp3rtp 224.17.23.42:5004:2 -b 56 - /dev/null\n"
            "      arecord -b 16 -s 44100 -w | ./mp3rtp 10.1.1.42 -V2 -b128 -B256 - "
            "my_mp3file.mp3\n"
            "\n" );
        return 1;
    }
     ...
     */
    {
        ...
        argv_mod[ 0 ] = argv[ 0 ];
        for ( i = 2; i < argc; ++i )
        { /* leaving out argument number 1, parsed above */
            argv_mod[ i - 1 ] = argv[ i ];
        }
    ...
    while ( ( wavsamples = get_audio( gf, Buffer ) ) > 0 )
    { /* read in 'wavsamples' samples */
