...
timestatus( const lame_global_flags* const gfp )
{
    ...
    int percent;
    double tmx, delta;
    int samp_rate = lame_get_out_samplerate( gfp ), frameNum = lame_get_frameNum( gfp ),
        totalframes = lame_get_totalframes( gfp ), framesize = lame_get_framesize( gfp );
    ...
    console_printf( proc_time->speed_index <= 1. ? "%9.4f" SPEED_CHAR "|" : "%#9.5g" SPEED_CHAR "|",
...
brhist_init_package( lame_global_flags const* gf )
{
    ...
    {
        if ( brhist_init(
                 gf, lame_get_VBR_min_bitrate_kbps( gf ), lame_get_VBR_max_bitrate_kbps( gf ) ) )
        {
            /* fail to initialize */
...
void
encoder_progress_begin( lame_global_flags const* gf, char const* inPath, char const* outPath )
{
...
encoder_progress( lame_global_flags const* gf )
{
    if ( global_ui_config.silent <= 0 )
    {
        ...
        { /*  most likely --disptime x not used */
            if ( frames_diff < 100 && frames_diff != 0 )
...
calcEndPadding( unsigned long samples, int pcm_samples_per_frame )
{
...
static unsigned long
calcNumBlocks( unsigned long samples, int pcm_samples_per_frame )
{
    unsigned long end_padding;
    samples += 576;
    end_padding = pcm_samples_per_frame - ( samples % pcm_samples_per_frame );
...
decoder_progress( DecoderProgress dp, const mp3data_struct* mp3data, int iread )
{
    ...
    console_printf(
        "\rFrame#%6i/%-6i %3i kbps", dp->frame_ctr, dp->frames_total, mp3data->bitrate );
