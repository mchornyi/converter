...
#if defined(_WIN32)
__declspec( thread ) get_audio_global_data global;
#else
...
static int lame_decode_fromfile( FILE* fd,
                                 short int pcm_l[],
                                 short int pcm_r[],
...
#define S_ISFIFO( m ) ( ( (m)&_S_IFIFO ) == _S_IFIFO )
    ...
    if ( whence != SEEK_CUR || offset < 0 )
    {
        ...
        {
            error_printf(
                "fskip problem: Mostly the return status of functions is not evaluate so it is "
                "more secure to polute <stderr>.\n" );
        }
        return -1;
    }
...
setSkipStartAndEnd( lame_t gfp, int enc_delay, int enc_padding )
{
    ...
    case sf_mp3:
        ...
        else
        {
            ...
            skip_start
                += 528 + 1; /* mp3 decoder has a 528 sample delay, plus user supplied "skip" */
        }
        break;
    case sf_mp2:
        skip_start += 240 + 1;
...
get_audio_common( lame_t gfp, int buffer[ 2 ][ 1152 ], short buffer16[ 2 ][ 1152 ] )
{
    ...
    if ( ( num_channels < 1 || 2 < num_channels ) || ( framesize < 1 || 1152 < framesize ) )
    {
        if ( global_ui_config.silent < 10 )
        {
...
read_samples_mp3( lame_t gfp, FILE* musicin, short int mpg123pcm[ 2 ][ 1152 ] )
{
    ...
    out = lame_decode_fromfile(
        musicin, mpg123pcm[ 0 ], mpg123pcm[ 1 ], &global_decoder.mp3input_data );
    /*
...
static int
set_input_num_channels( lame_t gfp, int num_channels )
{
    if ( gfp )
    {
...
static int
set_input_samplerate( lame_t gfp, int input_samplerate )
{
    if ( gfp )
    {
...
open_snd_file( lame_t gfp, char const* inPath )
{
...

    {
        ...
        if ( gs_pSndFileIn == NULL )
        {
            ...
            gs_wfInfo.seekable = ( global_reader.input_format != sf_raw ); /* if user specified -r,
                                                                              set to not seekable */
            ...
            if ( ( global_raw_pcm.in_endian == ByteOrderLittleEndian )
...
*/
static int
unpack_read_samples( const int samples_to_read,
...
#define GA_URS_IFLOOP( ga_urs_bps )       \
    ...
    if ( swap_order == 0 )
    {
        ...
        *--op = ip[ i ] << ( b - 24 ) | ip[ i + 1 ] << ( b - 16 ) | ip[ i + 2 ] << ( b - 8 );
        GA_URS_IFLOOP( 4 )
        *--op = ip[ i ] << ( b - 32 ) | ip[ i + 1 ] << ( b - 24 ) | ip[ i + 2 ] << ( b - 16 )
                | ip[ i + 3 ] << ( b - 8 );
    }
    else
    {
...
read_samples_pcm( FILE* musicin, int sample_buffer[ 2304 ], int samples_to_read )
{
    ...
    samples_read = unpack_read_samples(
        samples_to_read, bytes_per_sample, swap_byte_order, sample_buffer, musicin );
    if ( ferror( musicin ) )
    {
        if ( global_ui_config.silent < 10 )
        {
...
static int
open_mpeg_file_part2(
    lame_t gfp, FILE* musicin, char const* inPath, int* enc_delay, int* enc_padding )
{
#ifdef HAVE_MPGLIB
    if ( -1
         == lame_decode_initfile( musicin, &global_decoder.mp3input_data, enc_delay, enc_padding ) )
    {
        if ( global_ui_config.silent < 10 )
        {
...
lame_decode_initfile( FILE* fd, mp3data_struct* mp3data, int* enc_delay, int* enc_padding )
{
    ...
    ret = hip_decode1_headersB(
        global.hip, buf, len, pcm_l, pcm_r, mp3data, enc_delay, enc_padding );
    if ( -1 == ret )
        return -1;
