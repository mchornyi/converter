...
gpk_graph_draw( GtkWidget* widget, /* plot on this widged */
                ...
                gdouble* ycord, /* data */
                gdouble xmn,
                gdouble ymn, /* coordinates of corners */
                gdouble xmx,
                gdouble ymx,
...
configure_event( GtkWidget* widget, GdkEventConfigure* event, gpointer data )
{
    ...
    if ( ( ppixmap = findpixmap( widget ) ) )
    {
        ...
        *ppixmap = gdk_pixmap_new(
            widget->window, widget->allocation.width, widget->allocation.height, -1 );
...
gpk_plot_new( int width, int height )
{
    ...
    gtk_signal_connect(
        GTK_OBJECT( pixmapbox ), "expose_event", (GtkSignalFunc)expose_event, NULL );
    gtk_signal_connect(
        GTK_OBJECT( pixmapbox ), "configure_event", (GtkSignalFunc)configure_event, NULL );
