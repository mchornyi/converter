...
char*
toLatin1( char const* s )
{
    return utf8ToLatin1( s );
}

unsigned short*
toUtf16( char const* s )
{
...
static int
evaluateArgument( char const* token, char const* arg, char* _EndPtr )
{
...
static int
getDoubleValue( char const* token, char const* arg, double* ptr )
{
    char* _EndPtr = 0;
...
static int
getIntValue( char const* token, char const* arg, int* ptr )
{
    char* _EndPtr = 0;
...
{ /* print long syntax help */
    ...
    wait_for( fp, lessmode );
    DEV_HELP( help_developer_switches( fp ); wait_for( fp, lessmode ); )
...
presets_set( lame_t gfp, int fast, int cbr, const char* preset_name, const char* ProgramName )
{
    ...
    if ( ( strcmp( preset_name, "help" ) == 0 ) && ( fast < 1 ) && ( cbr < 1 ) )
    {
    ...
    if ( ( strcmp( preset_name, "phon+" ) == 0 ) || ( strcmp( preset_name, "lw" ) == 0 )
...
{
    char const* suffixes[]
        = {".WAV", ".RAW", ".MP1",  ".MP2", ".MP3", ".MPG", ".MPA", ".CDA", ".OGG", ".AIF", ".AIFF",
           ".AU",  ".SND", ".FLAC", ".WV",  ".OFR", ".TAK", ".MP4", ".M4A", ".PCM", ".W64"};
    size_t i;
...
static int
dev_only_with_arg(
    char const* str, char const* token, char const* nextArg, int* argIgnored, int* argUsed )
{
    if ( 0 != local_strcasecmp( token, str ) )
        return 0;
    *argUsed = 1;
    if ( internal_opts_enabled )
        return 1;
...
static int
dev_only_without_arg( char const* str, char const* token, int* argIgnored )
{
    if ( 0 != local_strcasecmp( token, str ) )
        return 0;
    if ( internal_opts_enabled )
...
#define T_ELIF( str )                               \
    ...
    else if ( 0 == local_strcasecmp( token, str ) ) \
    {
#define T_ELIF2( str1, str2 )                                                                \
    }                                                                                        \
    else if ( 0 == local_strcasecmp( token, str1 ) || 0 == local_strcasecmp( token, str2 ) ) \
    {
#define T_ELSE \
    }          \
    else       \
...
#define T_ELIF_INTERNAL_WITH_ARG( str )                                         \
    ...
    {
static int
parse_args_( lame_global_flags* gfp,
             int argc,
             char** argv,
             char* const inPath,
             char* const outPath,
             ...
             int* num_nogap )
{
    ...
    for ( i = 0; ++i < argc; )
    {
        ...
        if ( *token++ == '-' )
        {
            ...
            { /* GNU style */
                ...
                T_ELIF( "abr" )
                /* values larger than 8000 are bps (like Fraunhofer), so it's strange to get 320000
                    ...
                    if ( 0 == ignore_tag_errors )
                    {
                        ...
                        else if ( ret == -2 )
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    error_printf(
                                        "Unknown ID3v1 genre: '%s'.  Setting ID3v1 genre to "
                                        "'Other'\n",
                                        nextArg );
                                }
                            }
                ...
                if ( argUsed )
                {
                    ...
                    else
                    {
                        ...
                        lame_set_lowpassfreq(
                            gfp,
                            (int)( double_value * ( double_value < 50. ? 1.e3 : 1.e0 ) + 0.5 ) );
                    }
                }
                ...
                if ( argUsed )
                {
                    ...
                    {
                        error_printf(
                            "Must specify lowpass width with --lowpass-width freq, freq >= 0.001 "
                            "kHz\n" );
                        return -1;
                    }
                    lame_set_lowpasswidth(
                        gfp, (int)( double_value * ( double_value < 16. ? 1.e3 : 1.e0 ) + 0.5 ) );
                }
                ...
                if ( argUsed )
                {
                    ...
                    else
                    {
                        ...
                        lame_set_highpassfreq(
                            gfp,
                            (int)( double_value * ( double_value < 16. ? 1.e3 : 1.e0 ) + 0.5 ) );
                    }
                }
                ...
                if ( argUsed )
                {
                    ...
                    {
                        error_printf(
                            "Must specify highpass width with --highpass-width freq, freq >= 0.001 "
                            "kHz\n" );
                        return -1;
                    }
            ...
            else
            {
                while ( ( c = *token++ ) != '\0' )
                {
                    ...
                    case 's':
                        ...
                        {
                            double_value
                                = (int)( double_value * ( double_value <= 192 ? 1.e3 : 1.e0 )
                                         + 0.5 );
    ...
    if ( outPath[ 0 ] == '\0' )
    { /* no explicit output dir or file */
        if ( count_nogap > 0 )
        { /* in case of nogap encode */
