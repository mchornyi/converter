...
brhist_disp_line( int i, int br_hist_TOT, int br_hist_LR, int full, int frames )
{
    ...
    if ( full != 0 )
    {
        /* some problems when br_hist_TOT \approx br_hist_LR: You can't see that there are still MS
         * frames */
        barlen_TOT
            = ( br_hist_TOT * ( Console_IO.disp_width - res ) + full - 1 ) / full; /* round up */
        barlen_LR
            = ( br_hist_LR * ( Console_IO.disp_width - res ) + full - 1 ) / full; /* round up */
    }
    else
    {
...
brhist_disp( const lame_global_flags* gf )
{
    ...
    int br_sm_hist[ BRHIST_WIDTH ][ 4 ]; /* how often a special frame size/stereo mode commbination
                                            was used */
    int st_mode[ 4 ];
    int bl_type[ 6 ];
    int frames;     /* total number of encoded frames */
    int most_often; /* usage count of the most often used frame size, but not smaller than
                       Console_IO.disp_width-BRHIST_RES (makes this sense?) and 1 */
    double sum = 0.;

    double stat[ 9 ] = {0};
    int st_frames = 0;

    brhist.hist_printed_lines = 0; /* printed number of lines for the brhist functionality, used to
                                      skip back the right number of lines */
