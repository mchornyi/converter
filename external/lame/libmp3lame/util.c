    ...
    if ( ptr )
    {
        if ( !ptr->pointer )
        {
            ...
            if ( ptr->pointer != 0 )
            {
                ...
                {
                    ptr->aligned
                        = (void*)( ( ( (size_t)ptr->pointer + bytes - 1 ) / bytes ) * bytes );
                }
                else
                {
...
ATHformula_GB( FLOAT f, FLOAT value, FLOAT f_min, FLOAT f_max )
{
    ...
    ath = 3.640 * pow( f, -0.8 ) - 6.800 * exp( -0.6 * pow( f - 3.4, 2.0 ) )
          + 6.000 * exp( -0.15 * pow( f - 8.7, 2.0 ) )
             ...
             int* n_out )
{
    ...
    if ( isResamplingNecessary( cfg ) )
    {
        do
        {
            nout = fill_buffer_resample(
                gfc, &mfbuf[ ch ][ mf_size ], framesize, in_buffer[ ch ], nsamples, n_in, ch );
        } while ( ++ch < nch );
        *n_out = nout;
    }
    else
    {
        ...
        {
            memcpy(
                &mfbuf[ ch ][ mf_size ], &in_buffer[ ch ][ 0 ], nout * sizeof( mfbuf[ 0 ][ 0 ] ) );
        } while ( ++ch < nch );
        *n_out = nout;
        *n_in = nout;
    }
