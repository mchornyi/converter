    ...
    case 0:
        better
            = calc->over_count < best->over_count
              ...
              || ( calc->over_count == best->over_count && EQ( calc->over_noise, best->over_noise )
...
static int
inc_subblock_gain( const lame_internal_flags* const gfc,
                   gr_info* const cod_info,
                   FLOAT xrpow[ 576 ] )
{
    int sfb, window;
...
static int
outer_loop( lame_internal_flags* gfc,
            gr_info* const cod_info,
...
{ /* maximum allowed bits */
    ...
    while ( !bEndOfSearch )
    {
        ...
        do
        {
             ...
             */
            while (
                ( cod_info_w.part2_3_length = count_bits( gfc, xrpow, &cod_info_w, &prev_noise ) )
                    > huff_bits
...
static void
VBR_encode_granule( lame_internal_flags* gfc,
                    gr_info* const cod_info,
...
void
VBR_old_iteration_loop( lame_internal_flags* gfc,
                        const FLOAT pe[ 2 ][ 2 ],
    ...
    for ( ;; )
    {
        ...
        for ( gr = 0; gr < cfg->mode_gr; gr++ )
        {
            for ( ch = 0; ch < cfg->channels_out; ch++ )
            {
                ...
                {
                    trancate_smallspectrums(
                        gfc, &l3_side->tt[ gr ][ ch ], l3_xmin[ gr ][ ch ], xrpow );
                }
...
void
VBR_new_iteration_loop( lame_internal_flags* gfc,
                        const FLOAT pe[ 2 ][ 2 ],
                        const FLOAT ms_ener_ratio[ 2 ],
                        const III_psy_ratio ratio[ 2 ][ 2 ] )
{
...
void
ABR_iteration_loop( lame_internal_flags* gfc,
                    const FLOAT pe[ 2 ][ 2 ],
                    const FLOAT ms_ener_ratio[ 2 ],
                    const III_psy_ratio ratio[ 2 ][ 2 ] )
{
...
void
CBR_iteration_loop( lame_internal_flags* gfc,
                    const FLOAT pe[ 2 ][ 2 ],
                    const FLOAT ms_ener_ratio[ 2 ],
                    const III_psy_ratio ratio[ 2 ][ 2 ] )
{
