...
k_34_4( DOUBLEX x[ 4 ], int l3[ 4 ] )
{
    ...
    assert( x[ 0 ] <= IXMAX_VAL && x[ 1 ] <= IXMAX_VAL && x[ 2 ] <= IXMAX_VAL
            && x[ 3 ] <= IXMAX_VAL );
    x[ 0 ] += MAGIC_FLOAT;
    fi[ 0 ].f = x[ 0 ];
    x[ 1 ] += MAGIC_FLOAT;
...
static uint8_t
tri_calc_sfb_noise_x34( const FLOAT* xr,
                        const FLOAT* xr34,
                        FLOAT l3_xmin,
                        unsigned int bw,
                        uint8_t sf,
...
static const uint8_t max_range_short[ SBMAX_s * 3 ]
    = {15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 7, 7,
...
static void
short_block_constrain( const algo_t* that,
                       const int vbrsf[ SFBMAX ],
...
static void
long_block_constrain( const algo_t* that,
                      const int vbrsf[ SFBMAX ],
                      const int vbrsfmin[ SFBMAX ],
                      int vbrmax )
{
...
static int
tryGlobalStepsize( const algo_t* that,
...
static void
searchGlobalStepsizeMax( const algo_t* that,
                         const int sfwork[ SFBMAX ],
                         const int vbrsfmin[ SFBMAX ],
                         int target )
{
...
static int
tryThatOne( algo_t const* that,
            const int sftemp[ SFBMAX ],
            const int vbrsfmin[ SFBMAX ],
            int vbrmax )
{
    FLOAT const xrpow_max = that->cod_info->xrpow_max;
...
static void
outOfBitsStrategy( algo_t const* that,
                   const int sfwork[ SFBMAX ],
                   const int vbrsfmin[ SFBMAX ],
                   int target )
{
...
int
VBR_encode_frame( lame_internal_flags* gfc,
                  const FLOAT xr34orig[ 2 ][ 2 ][ 576 ],
                  const FLOAT l3_xmin[ 2 ][ 2 ][ SFBMAX ],
                  const int max_bits[ 2 ][ 2 ] )
{
    SessionConfig_t const* const cfg = &gfc->cfg;
    ...
    for ( gr = 0; gr < ngr; ++gr )
    {
        ...
        for ( ch = 0; ch < nch; ++ch )
        {
            ...
            that_[ gr ][ ch ].find
                = ( cfg->full_outer_loop < 0 ) ? guess_scalefac_x34 : find_scalefac_x34;
            that_[ gr ][ ch ].gfc = gfc;
