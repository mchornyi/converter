...
static const int genre_alpha_map[]
    = {123, 34, 74,  73,  99,  20,  40,  26,  145, 90,  116, 41,  135, 85,  96,  138, 89,  0,  107,
       132, 65, 88,  104, 102, 97,  136, 61,  141, 32,  1,   112, 128, 57,  140, 2,   139, 58, 3,
       125, 50, 22,  4,   55,  127, 122, 120, 98,  52,  48,  54,  124, 25,  84,  80,  115, 81, 119,
       5,   30, 36,  59,  126, 38,  49,  91,  6,   129, 79,  137, 7,   35,  100, 131, 19,  33, 46,
       47,  8,  29,  146, 63,  86,  71,  45,  142, 9,   77,  82,  64,  133, 10,  66,  39,  11, 103,
       12,  75, 134, 13,  53,  62,  109, 117, 23,  108, 92,  67,  93,  43,  121, 15,  68,  14, 16,
       76,  87, 118, 17,  78,  143, 114, 110, 69,  21,  111, 95,  105, 42,  37,  24,  56,  44, 101,
       83,  94, 106, 147, 113, 18,  51,  130, 144, 60,  70,  31,  72,  27,  28};
...
typedef enum MiscIDs
{
    ...
    ,
    ID_WFED
    = FRAME_ID( 'W', 'F', 'E', 'D' ) /* iTunes Podcast URL as TEXT FRAME !!! violates standard */
} MiscIDs;
...
static int id3v2_add_ucs2_lng( lame_t gfp,
                               uint32_t frame_id,
                               unsigned short const* desc,
                               unsigned short const* text );
static int id3v2_add_latin1_lng( lame_t gfp,
                                 uint32_t frame_id,
                                 char const* desc,
                                 char const* text );
...
local_strdup( char** dst, const char* src )
{
    ...
    *dst = 0;
    if ( src != 0 )
    {
        ...
        if ( n > 0 )
        { /* string length without zero termination */
    ...
    if ( image != 0 )
    {
        ...
        if ( 2 < size && data[ 0 ] == 0xFF && data[ 1 ] == 0xD8 )
        {
            mimetype = MIMETYPE_JPEG;
                  ...
                  && strncmp( (const char*)&data[ 1 ], "PNG", 3 ) == 0 )
        {
...
static int
id3v2_add_latin1(
    lame_t gfp, uint32_t frame_id, char const* lng, char const* desc, char const* text )
{
    lame_internal_flags* gfc = gfp != 0 ? gfp->internal_flags : 0;
    if ( gfc != 0 )
    {
...
id3tag_set_textinfo_utf16( lame_t gfp, char const* id, unsigned short const* text )
{
    ...
    if ( frame_id == ID_WFED )
    {
        return id3v2_add_ucs2_lng(
            gfp, frame_id, text, 0 ); /* iTunes expects WFED to be a text frame */
    }
...
int
id3tag_set_comment_utf16( lame_t gfp,
                          char const* lang,
                          unsigned short const* desc,
                          unsigned short const* text )
{
...
extern int id3tag_set_comment_ucs2( lame_t gfp,
                                    char const* lang,
                                    unsigned short const* desc,
                                    unsigned short const* text );

int
id3tag_set_comment_ucs2( lame_t gfp,
                         char const* lang,
                         unsigned short const* desc,
                         unsigned short const* text )
{
...
static const char*
nextUpperAlpha( const char* p, char x )
{
    char c;
...
lame_get_id3v2_tag( lame_t gfp, unsigned char* buffer, size_t size )
{
...
#endif
    {
        ...
        if ( ( title_length > 30 ) || ( artist_length > 30 ) || ( album_length > 30 )
             || ( comment_length > 30 )
        ...
        if ( usev2 )
        {
            ...
            *p++ = (unsigned char)( adjusted_tag_size & 0x7fu );
            ...
            {
                p = set_frame_apic(
                    p, albumart_mime, gfc->tag_spec.albumart, gfc->tag_spec.albumart_size );
            }
