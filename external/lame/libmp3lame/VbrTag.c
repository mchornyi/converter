...
IsVbrTag( const unsigned char* buf )
{
    ...
    isTag0 = ( ( buf[ 0 ] == VBRTag0[ 0 ] ) && ( buf[ 1 ] == VBRTag0[ 1 ] )
               && ( buf[ 2 ] == VBRTag0[ 2 ] ) && ( buf[ 3 ] == VBRTag0[ 3 ] ) );
    isTag1 = ( ( buf[ 0 ] == VBRTag1[ 0 ] ) && ( buf[ 1 ] == VBRTag1[ 1 ] )
               && ( buf[ 2 ] == VBRTag1[ 2 ] ) && ( buf[ 3 ] == VBRTag1[ 3 ] ) );
 ...
 *                              crc                             : computation of crc-16 of Lame Tag
 *so far (starting at frame sync)
 *
 ****************************************************************************
 */
static int
PutLameVBR( lame_global_flags const* gfp,
            size_t nMusicLength,
            ...
            uint16_t crc )
{
    ...
    */
    ...
    nFlags = nAthType + ( bExpNPsyTune << 4 ) + ( bSafeJoint << 5 ) + ( bNoGapMore << 6 )
             + ( bNoGapPrevious << 7 );
    ...
    if ( cfg->short_blocks == short_block_forced || cfg->short_blocks == short_block_dispensed
         ...
         ( cfg->disable_reservoir && cfg->avg_bitrate < 320 ) || cfg->noATH || cfg->ATHonly
         || ( nAthType == 0 ) || cfg->samplerate_in <= 32000 )
        bNonOptimal = 1;

    nMisc = nNoiseShaping + ( nStereoMode << 2 ) + ( bNonOptimal << 5 ) + ( nSourceFreq << 6 );
