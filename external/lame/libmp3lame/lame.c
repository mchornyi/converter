...
optimum_bandwidth( double* const lowerlimit, double* const upperlimit, const unsigned bitrate )
{
    ...
    const band_pass_t freq_map[] = {{8, 2000},
                                    {16, 3700},
                                    {24, 3900},
                                    {32, 5500},
                                    {40, 7000},
...
lame_init_params( lame_global_flags* gfp )
{
    ...
    gfc->lame_init_params_successful
        = 0; /* will be set to one, when we get through until the end */
    ...
    if ( cfg->vbr == vbr_off && gfp->compression_ratio > 0 )
    {
         ...
         */
        gfp->brate
            = gfp->samplerate_out * 16 * cfg->channels_out / ( 1.e3 * gfp->compression_ratio );
    ...
    if ( gfp->samplerate_out == 0 && ( cfg->vbr == vbr_mt || cfg->vbr == vbr_mtrh ) )
    {
        ...
        struct q_map const m[ 9 ] = {{48000, 0.0, 6.5, 0.0, 6.5, 23700},
    ...
    if ( cfg->vbr == vbr_off )
    {
        gfp->compression_ratio
            = gfp->samplerate_out * 16 * cfg->channels_out / ( 1.e3 * gfp->brate );
    }
    if ( cfg->vbr == vbr_abr )
    {
        gfp->compression_ratio
    ...
    { /* choose a min/max bitrate for VBR */
        ...
        if ( gfp->VBR_min_bitrate_kbps )
        {
            gfp->VBR_min_bitrate_kbps = FindNearestBitrate(
                gfp->VBR_min_bitrate_kbps, cfg->version, cfg->samplerate_out );
        ...
        if ( gfp->VBR_max_bitrate_kbps )
        {
            gfp->VBR_max_bitrate_kbps = FindNearestBitrate(
                gfp->VBR_max_bitrate_kbps, cfg->version, cfg->samplerate_out );
    ...
    {
        if ( InitGainAnalysis( gfc->sv_rpg.rgdata, cfg->samplerate_out )
...
lame_print_config( const lame_global_flags* gfp )
{
...
#endif
    if ( gfc->CPU_features.MMX || gfc->CPU_features.AMD_3DNow || gfc->CPU_features.SSE
         || gfc->CPU_features.SSE2 )
    {
        char text[ 256 ] = {0};
        int fft_asm_used = 0;
#ifdef HAVE_NASM
    ...
    if ( cfg->free_format )
    {
        ...
        {
            MSGF( gfc,
                  "Warning: many decoders cannot handle free format bitrates >320 kbps (see "
                  "documentation)\n" );
        }
    }
}
...
lame_print_internals( const lame_global_flags* gfp )
{
    ...
    MSGF( gfc,
          ...
          10 * log10( gfc->sv_qnt.longfact[ 14 ] ),
                             ...
                             const int mp3buf_size )
{
    ...
    while ( nsamples > 0 )
    {
        ...
        if ( cfg->findReplayGain && !cfg->decode_on_the_fly )
            if ( AnalyzeSamples( gfc->sv_rpg.rgdata,
                                 &mfbuf[ 0 ][ esv->mf_size ],
                                 &mfbuf[ 1 ][ esv->mf_size ],
                                 n_out,
                                 cfg->channels_out )
...
lame_encode_buffer( lame_global_flags* gfp,
                    const short int pcm_l[],
                    const short int pcm_r[],
                    const int nsamples,
                    unsigned char* mp3buf,
                    const int mp3buf_size )
{
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, nsamples, mp3buf, mp3buf_size, pcm_short_type, 1, 1.0 );
}
                          ...
                          const int mp3buf_size )
{
    ...
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, nsamples, mp3buf, mp3buf_size, pcm_float_type, 1, 1.0 );
}
                               ...
                               const int mp3buf_size )
{
    ...
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, nsamples, mp3buf, mp3buf_size, pcm_float_type, 1, 32767.0 );
}
                                           ...
                                           const int mp3buf_size )
{
    /* input is assumed to be normalized to +/- 1.0 for full scale */
    return lame_encode_buffer_template(
        gfp, pcm, pcm + 1, nsamples, mp3buf, mp3buf_size, pcm_float_type, 2, 32767.0 );
}
                                ...
                                const int mp3buf_size )
{
    /* input is assumed to be normalized to +/- 1.0 for full scale */
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, nsamples, mp3buf, mp3buf_size, pcm_double_type, 1, 32767.0 );
}
                                            ...
                                            const int mp3buf_size )
{
    /* input is assumed to be normalized to +/- 1.0 for full scale */
    return lame_encode_buffer_template(
        gfp, pcm, pcm + 1, nsamples, mp3buf, mp3buf_size, pcm_double_type, 2, 32767.0 );
}
                        ...
                        const int mp3buf_size )
{
    ...
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, nsamples, mp3buf, mp3buf_size, pcm_int_type, 1, norm );
}
                          ...
                          const int mp3buf_size )
{
    ...
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, nsamples, mp3buf, mp3buf_size, pcm_long_type, 1, norm );
}
                         ...
                         const int mp3buf_size )
{
    /* input is assumed to be normalized to +/- 32768 for full scale */
    return lame_encode_buffer_template(
        gfp, pcm_l, pcm_r, nsamples, mp3buf, mp3buf_size, pcm_long_type, 1, 1.0 );
}

int
lame_encode_buffer_interleaved(
    lame_global_flags* gfp, short int pcm[], int nsamples, unsigned char* mp3buf, int mp3buf_size )
{
    /* input is assumed to be normalized to +/- MAX_SHORT for full scale */
    return lame_encode_buffer_template(
        gfp, pcm, pcm + 1, nsamples, mp3buf, mp3buf_size, pcm_short_type, 2, 1.0 );
}

int
lame_encode_buffer_interleaved_int(
    lame_t gfp, const int pcm[], const int nsamples, unsigned char* mp3buf, const int mp3buf_size )
{
    /* input is assumed to be normalized to +/- MAX(int) for full scale */
    FLOAT const norm = ( 1.0 / ( 1L << ( 8 * sizeof( int ) - 16 ) ) );
    return lame_encode_buffer_template(
        gfp, pcm, pcm + 1, nsamples, mp3buf, mp3buf_size, pcm_int_type, 2, norm );
}
...
lame_init_bitstream( lame_global_flags* gfp )
{
    if ( is_lame_global_flags_valid( gfp ) )
    {
        ...
        if ( gfc != 0 )
        {
            ...
            memset( gfc->ov_enc.bitrate_channelmode_hist,
                    0,
                    sizeof( gfc->ov_enc.bitrate_channelmode_hist ) );
            memset( gfc->ov_enc.bitrate_blocktype_hist,
                    0,
...
lame_encode_flush( lame_global_flags* gfp, unsigned char* mp3buffer, int mp3buffer_size )
{
    ...
    while ( frames_left > 0 && imp3 >= 0 )
    {
        ...
        imp3 = lame_encode_buffer(
            gfp, buffer[ 0 ], buffer[ 1 ], bunch, mp3buffer, mp3buffer_size_remaining );
...
int CDECL lame_encode_finish( lame_global_flags* gfp,
        ...
        if ( is_lame_internal_flags_valid( gfc ) )
        {
            ...
            else
            {
                for ( j = 0; j < 14; j++ )
                {
                    for ( i = 0; i < 4; i++ )
                    {
                        bitrate_stmode_count[ j ][ i ]
                            = eov->bitrate_channelmode_hist[ j + 1 ][ i ];
                    }
                }
            }
        }
