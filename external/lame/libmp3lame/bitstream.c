...
get_max_frame_buffer_size_by_constraint( SessionConfig_t const* cfg, int constraint )
{
    ...
    else
    {
        ...
        case MDB_DEFAULT:
             ...
             */
            /* All mp3 decoders should have enough buffer to handle this value: size of a 320kbps
             * 32kHz frame */
            maxmp3buf = 8 * 1440;
            break;
        case MDB_STRICT_ISO:
...
#ifdef DEBUG
    DEBUGF( gfc,
            "count1: real: %ld counted:%d (bigv %d count1len %d)\n",
            gfc->bs.totbit - gegebo,
            gi->count1bits,
            gi->big_values,
            gi->count1 );
#endif
...
/*
  ...
  */
inline static int
Huffmancode( lame_internal_flags* const gfc,
...
flush_bitstream( lame_internal_flags* gfc )
{
    ...
    assert( esv->header[ last_ptr ].write_timing + getframebits( gfc ) == gfc->bs.totbit );
...
do_gain_analysis( lame_internal_flags* gfc, unsigned char* buffer, int minimum )
{
    ...
    { /* decode the frame */
        ...
        {
            samples_out
                = hip_decode1_unclipped( gfc->hip, buffer, mp3_in, pcm_buf[ 0 ], pcm_buf[ 1 ] );
