...
#define plotting_data_defined
    ...
    typedef struct
    {
        ...
        FLOAT bo_weight[ Max( SBMAX_l, SBMAX_s ) ]; /* band weight long scalefactor bands, at
                                                       transition */
...
#define MFSIZE ( 3 * 1152 + ENCDELAY - MDCTDELAY )
    ...
    typedef struct
    {
        ...
        int avg_bitrate;
        int enforce_min_bitrate; /* strictly enforce VBR_min_bitrate normaly, it will be violated
                                    for analog silence */
...
#define LAME_ID 0xFFF88E3B
        ...
        void ( *init_xrpow_core )( gr_info* const cod_info,
                                   FLOAT xrpow[ 576 ],
                                   int upper,
                                   FLOAT* sum );
...
#define lame_internal_flags_defined
    ...
    int hip_decode1_unclipped(
        hip_t hip, unsigned char* mp3buf, size_t len, sample_t pcm_l[], sample_t pcm_r[] );
