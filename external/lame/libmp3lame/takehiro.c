    ...
    for ( sfb = 0; sfb <= sfbmax; sfb++ )
    {
        ...
        { /*should compute this part */
            ...
            if ( ( j + cod_info->width[ sfb ] ) > cod_info->max_nonzero_coeff )
            {
                ...
                memset( &pi[ cod_info->max_nonzero_coeff ],
                        0,
            ...
            if ( prev_noise && prev_noise->sfb_count1 > 0 && sfb >= prev_noise->sfb_count1
...
static const int huf_tbl_noESC[] = {1, 2, 5, 7, 7, 10, 10, 13, 13, 13, 13, 13, 13, 13, 13};
...
*/
static int
count_bit_null( const int* ix, const int* end, int max, unsigned int* s )
{
    (void)ix;
    (void)end;
    (void)max;
...
static const count_fnc count_fncs[] = {&count_bit_null,
                                       &count_bit_noESC,
                     ...
                     III_side_info_t* const l3_side )
{
    ...
    if ( cfg->mode_gr == 2 && gr == 1 && l3_side->tt[ 0 ][ ch ].block_type != SHORT_TYPE
...
static const int scale_long[ 16 ] = {0, 10, 20, 30, 33, 21, 31, 41, 32, 42, 52, 43, 53, 63, 64, 74};
