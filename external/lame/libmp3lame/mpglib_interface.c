...
int CDECL lame_decode_headers(
    unsigned char* mp3buf, int len, short pcm_l[], short pcm_r[], mp3data_struct* mp3data );
    ...
    int ( *decodeMP3_ptr )( PMPSTR, unsigned char*, int, char*, int, int* ) )
{
    ...
    if ( pmp->header_parsed || pmp->fsizeold > 0 || pmp->framesize > 0 )
    {
        ...
        if ( pmp->fsizeold > 0 ) /* works for free format and fixed, no overrun, temporal results
                                    are < 400.e6 */
            mp3data->bitrate
                = 8 * ( 4 + pmp->fsizeold ) * mp3data->samplerate / ( 1.e3 * mp3data->framesize )
                  + 0.5;
        else if ( pmp->framesize > 0 )
            mp3data->bitrate
                = 8 * ( 4 + pmp->framesize ) * mp3data->samplerate / ( 1.e3 * mp3data->framesize )
                  + 0.5;
        else
            mp3data->bitrate
                       ...
                       int* enc_padding )
{
    ...
    return decode1_headersB_clipchoice( &mp,
                                        buffer,
                                        len,
                                        (char*)pcm_l,
                                        (char*)pcm_r,
...
lame_decode1_headers(
    unsigned char* buffer, int len, short pcm_l[], short pcm_r[], mp3data_struct* mp3data )
{
    ...
    unsigned char* buffer, int len, short pcm_l[], short pcm_r[], mp3data_struct* mp3data )
{
    ...
    {
        switch ( ret
                 = lame_decode1_headers( buffer, len, pcm_l + totsize, pcm_r + totsize, mp3data ) )
        {
        case -1:
            return ret;
        case 0:
...
hip_decode1_unclipped(
    hip_t hip, unsigned char* buffer, size_t len, sample_t pcm_l[], sample_t pcm_r[] )
{
    ...
    if ( hip )
    {
        return decode1_headersB_clipchoice( hip,
                                            buffer,
                                            len,
                                            (char*)pcm_l,
                                            (char*)pcm_r,
                     ...
                     mp3data_struct* mp3data )
{
    ...
    return hip_decode1_headersB(
        hip, buffer, len, pcm_l, pcm_r, mp3data, &enc_delay, &enc_padding );
}
                    ...
                    mp3data_struct* mp3data )
{
    ...
    for ( ;; )
    {
        switch ( ret = hip_decode1_headers(
                     hip, buffer, len, pcm_l + totsize, pcm_r + totsize, mp3data ) )
        {
        case -1:
            return ret;
        case 0:
...
int
hip_decode1_headersB( hip_t hip,
                      unsigned char* buffer,
                      size_t len,
                      short pcm_l[],
                      short pcm_r[],
...
void
hip_set_pinfo( hip_t hip, plotting_data* pinfo )
{
    if ( hip )
    {
        hip->pinfo = pinfo;
...
void
hip_set_errorf( hip_t hip, lame_report_function func )
{
    if ( hip )
    {
        hip->report_err = func;
...
void
hip_set_debugf( hip_t hip, lame_report_function func )
{
    if ( hip )
    {
        hip->report_dbg = func;
...
void
hip_set_msgf( hip_t hip, lame_report_function func )
{
    if ( hip )
    {
        hip->report_msg = func;
